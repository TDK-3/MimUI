/* ============================================================================
 * Mim canvas demo — plain HTML5 canvas, zero external libraries
 * ---------------------------------------------------------------------------
 * The demo app: multiple windows, every widget type,
 * scrolling, live styling/toggles, custom drawing via gui.renderer,
 * background/foreground layers, and a modal.
 * ========================================================================== */
(function () {
  'use strict';
  ...
  let modalOpen = false;
  let demoAccent = [79, 140, 255, 255];  // Color tab
  let demoVol = 0.35;                     // Widgets tab grabber
  let autoTextLines = 3;
  let winRounding = 8;
  let frameRounding = 5;
  let winBorder = 1;
  ...
  const logs = [];
  function log(msg) { logs.push(msg); if (logs.length > 40) logs.shift(); }

  const allWindowTitles = ['Playground', 'Layout', 'Custom drawing', 'Settings', 'Styled', 'AutoResize', 'Addons', 'Inspector', 'Console', 'Style'];
  function toggleWindow(t) { const w = gui.getWindow(t); if (w) w.open = !w.open; }
  function setAllWindows(open) { for (const t of allWindowTitles) { const w = gui.getWindow(t); if (w) w.open = open; } }
  function saveLayout() {
    const data = {};
    for (const t of allWindowTitles) {
      const st = gui.state.windowStates.get(t);
      if (st) data[t] = [Math.round(st.x), Math.round(st.y), Math.round(st.w), Math.round(st.h)];
    }
    try { navigator.clipboard && navigator.clipboard.writeText(JSON.stringify(data, null, 2)); } catch (e) { /* ignore */ }
  ...
  function setupAppMenuBar() {
    gui.setAppMenuBar([
      { label: 'File', items: [
        ...
        { sep: true },
        { label: 'Open all windows', onActivated: () => { setAllWindows(true); log('opened all windows'); } },
        { label: 'Hide all windows', onActivated: () => { setAllWindows(false); log('hid all windows'); } },
      ]},
      { label: 'Windows', items: allWindowTitles.map((t) => ({
        label: t,
        selected: () => !!gui.getWindow(t) && gui.isWindowOpen(t),
        onActivated: () => { toggleWindow(t); log((gui.isWindowOpen(t) ? 'show ' : 'hide ') + t); },
      })) },
      { label: 'Layout', items: [
        {
        ...
        { label: 'Undock', onActivated: () => { if (gui.undock('Inspector', 'Console')) log('undocked Inspector + Console'); } },
        { sep: true },
        {
          label: 'Screen dock: Addons \u2192 left edge',
          selected: () => { const w = gui.getWindow('Addons'); return !!(w && w._edge === 'left'); },
          onActivated: () => { if (gui.dockToEdge('Addons', 'left')) log('Addons docked to left screen edge'); },
        },
        {
          label: 'Screen undock: Addons',
          onActivated: () => { if (gui.undockEdge('Addons')) log('Addons freed from the screen edge'); },
        },
        { sep: true },
        {
          label: 'Docking (drag grids + screen edges)',
          selected: () => gui.flags.docking,
          onActivated: () => { gui.flags.docking = !gui.flags.docking; log('interactive docking ' + (gui.flags.docking ? 'on' : 'off')); },
        },
      ]},
      { label: 'Settings', items: [
        { label: 'Edit Preferences', onActivated: () => { toggleWindow('Settings'); log('opened Settings'); } },
        { label: 'Toggle theme', onActivated: () => { settings.theme = settings.theme ? 0 : 1; applyTheme(); } },
      ]},
    ], { pos: 'top' });
  }
  ...
  function drawPlayground() {
    ...
    if (gui.button('Toast success')) gui.addons.notifs.toast('mim notifications work', { type: 'success' });
    gui.sameLine(12);
    if (gui.button('Toast error')) gui.addons.notifs.toast('something went wrong', { type: 'error' });
    ...
    if (gui.selectable('selectable (click to toggle)', selectableOn)) selectableOn = !selectableOn;
    ...
    if (gui.collapsingHeader('collapsing header')) {
      ...
      gui.textWrapped('Immediate mode GUIs redraw everything every frame; state lives in the library, keyed by ids — not in the view. Duplicate labels are fine.', { maxWidth: 260 });
    }
  ...
  function drawLayout() {
    ...
    if (gui.beginTabItem('Tree')) {
      if (gui.treeNode('root')) {
        gui.text('child text');
        if (gui.treeNode('nested')) { gui.text('deep'); gui.treePop(); }
        gui.treePop();
      }
      gui.endTabItem();
    }
  ...
  function drawCustomDrawing() {
    ...
    r.polyline([x0, y0 + 96, x0 + 40, y0 + 66, x0 + 80, y0 + 90, x0 + 120, y0 + 56, x0 + 160, y0 + 82], [140, 255, 160, 255], 2);
    r.fillPolygon([x0 + 188, y0 + 50, x0 + 213, y0 + 85, x0 + 238, y0 + 50], [255, 200, 80, 255]);
    r.fillRoundedRect(x0 + 248, y0 + 40, 30, 30, 6, [200, 140, 255, 255]);
    r.line(x0, y0 + 106, x0 + 280, y0 + 106, [120, 120, 130, 255], 1);
    r.drawText(x0 + 2, y0 + 114, 'layer: gui — drag this window while it animates', [200, 200, 210, 255], { fontSize: 12 });
  ...
  function drawSettings() {
    ...
    settings.rightClickNumeric = gui.checkbox('right-click numeric entry', settings.rightClickNumeric);
    settings.windowContextMenu = gui.checkbox('window context menus (right-click titles)', settings.windowContextMenu);
    settings.keyboardNavigation = gui.checkbox('keyboard navigation (Tab / Enter / arrows)', settings.keyboardNavigation);
    settings.resizeBarProximity = gui.sliderInt('resize bar proximity (0 = off)', settings.resizeBarProximity, 0, 16);
    settings.debug = gui.checkbox('debug overlay', settings.debug);
    syncSettingsToGui();

    gui.separatorText('Try the new interactions');
    gui.textWrapped(
      'Right-click a title bar, dock title, or member header for a context menu. ' +
      'Drag a dock member\'s slim header out to undock it. Drag any window: a join grid ' +
      'pops up over the window under the cursor — drop directly on one of its ' +
      'triangles to combine — and the screen-center grid (its triangles select ' +
      'a little beyond their drawn size) docks the window to that screen edge ' +
      '(the center apex is a plain drop). NoDock windows show no docking UI. Hover near a window\'s ' +
      'edge: a resize bar fades in and drags to scale that side (the corner grip ' +
      'resizes both at once and beats the scrollbar). Docks resize from all four ' +
      'edges. Screen-edge stacks never cover each other (columns own the full ' +
      'height); hover near a stack\'s inner edge — the side not docked — for a ' +
      'resize bar that scales the stack in its dock direction only. Combined ' +
      'windows (docks) are dockable too: drag one to a screen edge, or drop a ' +
      'window onto a globally docked one — the pair combines in place, both ' +
      'keeping the docked width (not the sum). Input follows what you see: the ' +
      'last-drawn element under the cursor is the only one hovered, clicked or ' +
      'dragged — a click (empty body included) only focuses that window; an actual ' +
      'drag raises it, so dragging one over the others always lands it on top.',
  ...
  function drawStyled() {
    if (!gui.beginWindow('Styled', {
      size: [228, 120], pos: [764, 386],
  ...
  function drawAuto() {
    if (!gui.beginWindow('AutoResize', { flags: Mim.WindowFlags.AutoResize, pos: [24, 664] })) return;
    for (let i = 0; i < autoTextLines; i++) gui.text('line ' + (i + 1) + ' — window sizes itself');
    gui.endWindow();
  }
  ...
  function drawStyle() {
    ...
    styleCols.windowBg = C.colorButton('window bg', styleCols.windowBg, { w: halfW, presets: false });
    gui.sameLine(12);
    styleCols.titleBg = C.colorButton('title bg', styleCols.titleBg, { w: halfW, presets: false });
    styleCols.border = C.colorButton('border', styleCols.border, { w: halfW, presets: false });
    gui.sameLine(12);
    styleCols.frameBg = C.colorButton('frame bg', styleCols.frameBg, { w: halfW, presets: false });
    styleVars.windowRounding = gui.sliderInt('window rounding', styleVars.windowRounding, 0, 24);
  ...
  function drawModal() {
    if (!modalOpen) return;
    if (!gui.beginWindow('Modal', {
      flags: Mim.WindowFlags.Modal | Mim.WindowFlags.AlwaysOnTop | Mim.WindowFlags.Closable | Mim.WindowFlags.FixedSize,
      size: [260, 140],
      onClose: () => { modalOpen = false; },
    })) { modalOpen = false; return; }
    gui.textWrapped('This modal blocks the windows it covers. A window drawn above it (AlwaysOnTop) stays clickable.');
    gui.dummy(10, 4);
  ...
  function drawAddons() {
    ...
    if (gui.beginTabItem('Curves')) {
      gui.addons.plots.plotBezier('bezier — drag the dots (points rescale with the window)', null, { share: 2 });
      gui.addons.plots.plotPolar('polar rose', (t) => 0.5 + 0.45 * Math.sin(4 * t), { share: 2 });
      gui.endTabItem();
    }
    if (gui.beginTabItem('Data')) {
      gui.addons.plots.plotBars('bars — hover a bar (negatives hang below zero)', barData, { share: 2, axisLabels: true });
      gui.addons.plots.plotSeries('two series — hover for values', seriesData, { share: 2, axisLabels: true });
      gui.endTabItem();
    }
    if (gui.beginTabItem('3D')) {
      gui.addons.t3d.plot3D('surface z = sin(2x)·cos(2y) — drag to rotate, wheel zooms',
        (x, y) => Math.sin(2 * x) * Math.cos(2 * y), { share: 1 });
      gui.endTabItem();
    }
    if (gui.beginTabItem('Table')) {
      gui.addons.tables.advancedTable('nodes — sort, select, filter, drag column edges',
        [
          { id: 'name', label: 'Name', width: 120 },
          { id: 'load', label: 'Load %', width: 80, align: 'right' },
          { id: 'temp', label: 'Temp °C', width: 80, align: 'right' },
        ],
        tableRows, { share: 1, filter: true });
      gui.endTabItem();
    }
    ...
    if (gui.beginTabItem('Color')) {
      // full-featured picker: SV pad, hue bar, alpha slider — the
      // preview box below re-themes live from the picked color
      demoAccent = gui.addons.color.colorButton('accent', demoAccent, { alpha: true });
      const pv = gui.getRegionAvail();
      if (pv.h > 34) {
        const pp = gui._nextPos();
        const pw = Math.min(150, pv.w);
        gui.renderer.fillRect(pp.x, pp.y, pw, 26, demoAccent);
        gui.renderer.strokeRoundedRect(pp.x + 0.5, pp.y + 0.5, pw - 1, 25, 4, gui._col('border'), 1);
        gui._drawText(pp.x + 8, pp.y + 7, gui.addons.color.toHex(demoAccent), gui._col('text'), gui._fo());
        gui._advance(pp.x, pp.y, pw, 26);
      }
      gui.text('alpha is on — watch the swatch checkerboard');
      gui.endTabItem();
    }
    if (gui.beginTabItem('Widgets')) {
      const W = gui.addons.widgets;
      demoVol = W.grabber('volume', demoVol, { min: 0, max: 1, w: 180 });
      W.spinner('rendering frame…');
      W.bullet('grabber: drag the bar, or click the track');
      W.bullet('spinner: animated from the gui clock');
      gui.addons.markdown.md('**bold**, *italic* and `code` — this block wraps to the window width', {});
      gui.endTabItem();
    }
    gui.endTabBar();
    gui.endWindow();
