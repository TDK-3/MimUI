...
(function () {
  ...
  const plotData = [
    0.2, 0.5, 0.35, 0.8, 0.55, 0.9, 0.4, 0.65, 0.3, 0.75, 0.5, 0.85,
  ];
  ...
  for (let i = 0; i < 10; i++) {
    const row = [];
    for (let j = 0; j < 16; j++)
      row.push(Math.sin(i * 0.65) * Math.cos(j * 0.42));
    heatData.push(row);
  }
          ...
          items: [
            ...
            { sep: true },
            {
              ...
              onActivated: () => {
                ...
                log(
                  "interactive docking " + (gui.flags.docking ? "on" : "off"),
                );
              },
            },
          ],
  ...
  function drawPlayground() {
    if (!gui.beginWindow("Playground", { size: [340, 430], pos: [24, 40] }))
      return;
    gui.text("Widgets — drag sliders, right-click for exact entry");
    gui.separator();

    if (gui.button("Press me"))
      gui.setTooltip("Buttons return true when clicked");
    gui.sameLine(8);
    gui.smallButton("sm");
    ...
    floatInput = gui.inputFloat("input float", floatInput, {
      min: -10,
      max: 10,
    });
  ...
  function drawLayout() {
    if (!gui.beginWindow("Layout", { size: [360, 344], pos: [384, 40] }))
      return;
  ...
  function drawCustomDrawing() {
    if (
      !gui.beginWindow("Custom drawing", { size: [256, 160], pos: [1000, 386] })
    )
      return;
  ...
  function drawSettings() {
    if (!gui.beginWindow("Settings", { size: [300, 310], pos: [384, 400] }))
      return;
    ...
    gui.dummy(10, 4);
    gui.text(
      'The "Styled" window shows per-window style overrides.',
      [160, 165, 175, 255],
    );
    gui.endWindow();
  }
  ...
  function drawAuto() {
    if (
      !gui.beginWindow("AutoResize", {
        flags: Mim.WindowFlags.AutoResize,
        pos: [24, 664],
      })
    )
      return;
   ...
   * addons/mim_color.js. */
  function drawStyle() {
    if (!gui.beginWindow("Style", { size: [300, 258], pos: [700, 452] }))
      return;
    ...
    styleCols.border = C.colorButton("border", styleCols.border, {
      w: halfW,
      presets: false,
    });
    gui.sameLine(12);
    ...
    styleVars.titleRounding = gui.sliderInt(
      "title rounding",
      styleVars.titleRounding,
      0,
      24,
    );
    styleVars.windowBorder = gui.sliderInt(
      "window border",
      styleVars.windowBorder,
      0,
      4,
    );
  ...
  function drawInspector() {
    if (!gui.beginWindow("Inspector", { size: [178, 170], pos: [24, 488] }))
      return;
    const s = gui.state;
    gui.text("hovered: " + (s.hoveredWindow ? s.hoveredWindow.title : "—"));
    gui.text(
      "top window: " +
        (s.zOrder.length ? s.zOrder[s.zOrder.length - 1].title : "—"),
    );
    gui.text("windows: " + s.zOrder.length + "    docks: " + s.docks.size);
    gui.text(
      "fps: " +
        Math.round(s.stats.fps) +
        "    " +
        s.stats.ms.toFixed(2) +
        " ms",
    );
    gui.separator();
  ...
  function drawConsole() {
    if (!gui.beginWindow("Console", { size: [178, 170], pos: [212, 488] }))
      return;
  ...
  function drawAddons() {
    if (!gui.beginWindow("Addons", { size: [492, 330], pos: [764, 40] }))
      return;
    ...
    if (gui.beginTabItem("Curves")) {
      ...
      gui.addons.plots.plotPolar(
        "polar rose",
        (t) => 0.5 + 0.45 * Math.sin(4 * t),
        { share: 2 },
      );
      gui.endTabItem();
    }
    if (gui.beginTabItem("Data")) {
      gui.addons.plots.plotBars(
        "bars — hover a bar (negatives hang below zero)",
        barData,
        { share: 2, axisLabels: true },
      );
      gui.addons.plots.plotSeries("two series — hover for values", seriesData, {
        share: 2,
        axisLabels: true,
      });
      gui.endTabItem();
    ...
    if (gui.beginTabItem("Table")) {
      gui.addons.tables.advancedTable(
        "nodes — sort, select, filter, drag column edges",
        [
    ...
    if (gui.beginTabItem("Heatmap")) {
      gui.addons.plots.plotHeatmap("field — 10×16 values", heatData, {
        share: 1,
      });
      gui.endTabItem();
    }
    if (gui.beginTabItem("Color")) {
      ...
      demoAccent = gui.addons.color.colorButton("accent", demoAccent, {
        alpha: true,
      });
      ...
      if (pv.h > 34) {
        ...
        gui.renderer.strokeRoundedRect(
          pp.x + 0.5,
          pp.y + 0.5,
          pw - 1,
          25,
        ...
        gui._drawText(
          pp.x + 8,
          pp.y + 7,
          gui.addons.color.toHex(demoAccent),
          gui._col("text"),
          gui._fo(),
        );
    ...
    if (gui.beginTabItem("Widgets")) {
      ...
      gui.addons.markdown.md(
        "**bold**, *italic* and `code` — this block wraps to the window width",
        {},
      );
      gui.endTabItem();
    }
  ...
  function loop() {
    ...
    gui.layer(Mim.Layers.Background, (r) => {
      ...
      r.fillRect(
        ...
        w,
        h,
        settings.theme === 0 ? [15, 16, 20, 255] : [238, 238, 242, 255],
      );
