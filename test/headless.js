...
test("basic frame + window + text + button click", () => {
  ...
  e.frame(draw);
  check(
    btnRect && btnRect.w > 20 && btnRect.h > 15,
    "button has a sensible rect " + JSON.stringify(btnRect),
  );
...
test("window resize grip clamps to min size", () => {
  ...
  check(
    w.w - w0 > 55,
    "width grew with drag, got " + w.w + " (was " + w0 + ")",
  );
...
test("window double-click resets position", () => {
  ...
  e.click(w.x + 100, w.y + 15);
  check(
    approx(w.x, 100, 0.01) && approx(w.y, 100, 0.01),
    "position restored, got " + w.x + "," + w.y,
  );
});
...
test("overlapping windows: topmost receives clicks only", () => {
  ...
  check(
    bClicks === 1 && aClicks === 0,
    "B got the click, not A (" + aClicks + "/" + bClicks + ")",
  );
  ...
  check(
    e.gui.state.zOrder[e.gui.state.zOrder.length - 1] === b,
    "clicking A did not raise it (B still topmost)",
  );
  ...
  check(
    aClicks === 1 && bClicks === 2,
    "focused A (below) does not steal the overlap click — B still topmost",
  );
  // an actual MOVE of A over B reorders the stack; then the overlap is A's
  e.dragTo(a.x + 100, a.y + 12, a.x + 150, a.y + 62, 10, 0);
  check(
    e.gui.state.zOrder[e.gui.state.zOrder.length - 1] === a,
    "moving A raised it to the front",
  );
  e.click(b.x + 20, b.y + b.titleH + 20);
...
test("pushId disambiguates repeated widgets in a loop", () => {
  ...
  check(
    clicked.length === 1 && clicked[0] === 2,
    "index 2 was the one clicked, got " + JSON.stringify(clicked),
  );
});
...
test("input text: type, select all, delete, copy/paste, undo", () => {
  ...
  check(
    Env.clipboard === "world!",
    "copied to clipboard, got " + JSON.stringify(Env.clipboard),
  );
  ...
  check(
    name === "world!",
    "paste replaced selection, got " + JSON.stringify(name),
  );
...
test("input text: double-click selects a word", () => {
  ...
  check(
    msg === "X quick fox",
    'double-click word select replaced "the", got ' + JSON.stringify(msg),
  );
});
...
test("input text: repeated backspace at end, arrow keys, caret stays in range", () => {
  ...
  check(
    name === "aX",
    "arrow positioning + insert, got " + JSON.stringify(name),
  );
...
test("gui.layer switches layer for the callback and restores it", () => {
  const e = new Env();
  const draw = () => {
    e.gui.layer(Mim.Layers.Background, (r) => {
      r.fillRect(0, 0, 100, 100, [1, 2, 3, 255]);
    });
  ...
  const layerAt = (idx) => {
    let l = "gui";
    for (let i = 0; i < idx; i++)
      if (calls[i][0] === "setLayer") l = calls[i][1];
    return l;
  };
  check(layerAt(fillIdx) === "background", "callback drew on background layer");
  check(
    layerAt(textIdx) === "gui",
    "window content drew on gui layer (restored)",
  );
...
test("scrolling: child region scrolls with wheel and clamps", () => {
  ...
  e.frame(draw);
  let child = null;
  for (const [k, w] of e.gui.state.windows)
    if (w.kind === "child") {
      child = w;
      break;
    }
  ...
  check(
    child.maxScrollY > 100,
    "child is scrollable, maxScrollY=" + child.maxScrollY,
  );
...
test("tabs switch and only the active tab draws content", () => {
  ...
  const draw = () => {
    ...
    if (e.gui.beginWindow("TB", { size: [300, 220] })) {
      ...
      if (e.gui.beginTabItem("One")) {
        e.gui.text("content one");
      }
      e.gui.endTabItem();
      if (e.gui.beginTabItem("Two")) {
        e.gui.text("content two");
      }
      e.gui.endTabItem();
      if (e.gui.beginTabItem("Three")) {
        e.gui.text("content three");
      }
      e.gui.endTabItem();
      e.gui.endTabBar();
      e.gui.endWindow();
  ...
  e.frame(draw);
  check(
    e.renderer.drawnText().includes("content one"),
    "first tab active by default",
  );
  check(
    !e.renderer.drawnText().includes("content two"),
    "second tab not drawn",
  );
  // click tab "Two"
  ...
  const texts = e.renderer.calls.filter(
    (c) => c[0] === "drawText" && c[3] === "Two",
  );
  ...
  check(
    e.renderer.drawnText().includes("content two"),
    "tab Two active after click",
  );
  check(
    !e.renderer.drawnText().includes("content one"),
    "tab One not drawn anymore",
  );
});

test("tab content keeps its size and scrolls 1:1 when the window scrolls", () => {
  const e = new Env();
  let availAtTabStart = -1;
  let cursorAtTabStart = -1;
  const draw = () => {
    availAtTabStart = -1;
    cursorAtTabStart = -1;
    if (e.gui.beginWindow("TBSC", { size: [300, 160] })) {
      e.gui.beginTabBar("tabs");
      if (e.gui.beginTabItem("One")) {
        // probe at the very start of the tab content: the available height
        // here is what fill/sharing plots use to size themselves, and the
        // SCREEN y is what a drawn plot's top edge follows
        availAtTabStart = e.gui.getRegionAvail().h;
        cursorAtTabStart = e.gui.getCursorScreenPos().y;
        for (let i = 0; i < 30; i++) e.gui.text("row " + i);
      }
      e.gui.endTabItem();
      if (e.gui.beginTabItem("Two")) {
        e.gui.text("two");
  ...
  e.frame(draw);
  const win = e.gui.getWindow("TBSC");
  check(
    win.maxScrollY > 0,
    "window is scrollable, maxScrollY=" + win.maxScrollY,
  );
  const availTop = availAtTabStart;
  const cursorTop = cursorAtTabStart;
  // scroll straight to the bottom (no wheel smoothing to wait for)
  win.scrollY = win.scrollTargetY = win.maxScrollY;
  e.frame(draw);
  const availBottom = availAtTabStart;
  const cursorBottom = cursorAtTabStart;
  check(
    approx(availTop, availBottom, 0.01),
    "tab region height unchanged by scroll (" +
      availTop.toFixed(1) +
      " -> " +
      availBottom.toFixed(1) +
      ")",
  );
  check(
    approx(cursorTop - cursorBottom, win.scrollY, 0.01),
    "tab content scrolls 1:1 with the window (moved " +
      (cursorTop - cursorBottom).toFixed(1) +
      " for scroll " +
      win.scrollY.toFixed(1) +
      ")",
  );
  // and back to the top: everything is exactly as before
  win.scrollY = win.scrollTargetY = 0;
  e.frame(draw);
  check(
    approx(availAtTabStart, availTop, 0.01),
    "back at top: tab region height restored",
  );
});
...
test("tooltip appears after hover delay and disappears on leave", () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    e.frame(draw);
  } // advance internal clock past the delay
  check(
    e.renderer.drawnText().includes("A helpful tip"),
    "tooltip drawn after delay",
  );
  e.hover(-100, -100);
  e.gui._timeOffset += 20;
  e.frame(draw);
...
test("style push/pop overrides colors for one block", () => {
  ...
  e.frame(draw);
  const red = e.renderer.calls.find(
    (c) =>
      c[0] === "fillRoundedRect" && c[6] && c[6][0] === 200 && c[6][1] === 30,
  );
...
test("custom layers: background before, foreground after", () => {
  ...
  e.frame(draw);
  const layers = e.renderer.calls
    .filter((c) => c[0] === "setLayer")
    .map((c) => c[1]);
...
test("per-window style overrides apply", () => {
  ...
  const draw = () => {
    if (
      e.gui.beginWindow("PW", {
        size: [220, 140],
        style: { bg: [10, 60, 120, 255], rounding: 18 },
      })
    ) {
      e.gui.endWindow();
    }
  ...
  const bg = e.renderer.calls.find(
    (c) => c[0] === "fillRoundedRect" && c[6] && c[6][2] === 120,
  );
...
test("window state persists across frames and reappears", () => {
  ...
  const draw = (show) => () => {
    if (show) {
      if (e.gui.beginWindow("Flicker", { size: [200, 120], pos: [50, 60] }))
        e.gui.endWindow();
    }
  };
  e.frame(draw(true));
  ...
  check(
    approx(w2.x, 150, 0.01) && approx(w2.y, 160, 0.01),
    "position restored from persisted state, got " + w2.x + "," + w2.y,
  );
});
...
test("tables render header and rows", () => {
  ...
  check(
    texts.includes("Name") && texts.includes("alpha") && texts.includes("beta"),
    "table contents drawn",
  );
});
...
test("tree nodes expand with indent and pop", () => {
  ...
  const nodeTxt = e.renderer.calls.find(
    (c) => c[0] === "drawText" && c[3] === "Node",
  );
  const nestedTxt = e.renderer.calls.find(
    (c) => c[0] === "drawText" && c[3] === "nested text",
  );
...
test("plotLines and progressBar draw without errors", () => {
  ...
  e.frame(draw);
  check(
    e.renderer.calls.some((c) => c[0] === "polyline"),
    "plot polyline drawn",
  );
...
test("setNextItemWidth constrains slider width", () => {
  ...
  check(
    r1.w < r2.w,
    "next-item width applied only to the next widget (" +
      r1.w +
      " vs " +
      r2.w +
      ")",
...
test("group treats contents as one layout block", () => {
  ...
  check(
    after.y > win.y + win.titleH + lineH * 2,
    "cursor advanced past full group height, y=" + after.y,
  );
});
...
test("sameLine places the next element on the previous element\u2019s line (automatic padding)", () => {
  ...
  check(
    Math.abs(r2.y - r1.y) < 0.01,
    "same y level as the previous element, got " + r1.y + " vs " + r2.y,
  );
  check(
    Math.abs(r2.x - (r1.x + r1.w + sp[0])) < 0.01,
    "x is to the right of the previous element + the horizontal item spacing, got gap " +
      (r2.x - (r1.x + r1.w)) +
      " (expected " +
      sp[0] +
      ")",
...
test("sameLine supports explicit spacing and an absolute line offset", () => {
  ...
  e.frame(draw);
  check(
    Math.abs(rb.x - (ra.x + ra.w + 24)) < 0.01,
    "explicit spacing honored, got gap " + (rb.x - (ra.x + ra.w)),
  );
  check(
    Math.abs(rb.y - ra.y) < 0.01,
    "explicit-spacing element stays on the line",
  );
  check(
    Math.abs(rd.x - (rc.x + 60)) < 0.01,
    "offsetX anchors from the line start, got x " +
      rd.x +
      " (line start " +
      rc.x +
      ")",
...
test("sameLine chains across a row, then the next element drops below the row", () => {
  ...
  e.frame(draw);
  check(
    Math.abs(rb.y - ra.y) < 0.01 && Math.abs(rc.y - ra.y) < 0.01,
    "chained elements share the line y, got " + [ra.y, rb.y, rc.y].join(","),
  );
  check(
    rd.y > rc.y + 10,
    "the element after the row starts a new line below it, got " +
      rd.y +
      " vs " +
      rc.y,
  );
  check(
    Math.abs(rd.x - ra.x) < 0.01,
    "the new line is left-aligned with the row, got " + rd.x + " vs " + ra.x,
  );
});
...
test("sameLine with no previous element is a no-op (if applicable)", () => {
  ...
  check(
    r2.y > r1.y + 10,
    "no stale request: the first two elements are on separate lines, got " +
      r1.y +
      " vs " +
      r2.y,
  );
  check(
    Math.abs(r2.x - r1.x) < 0.01,
    "the second element is left-aligned (not merged), got " +
      r2.x +
      " vs " +
      r1.x,
  );
...
test("an empty group after sameLine does not leak the request", () => {
  ...
  check(
    rb.y > ra.y + 10,
    "the empty group consumed the same-line slot; b is on the next line, got (" +
      rb.x +
      "," +
      rb.y +
      ") vs (" +
  ...
  check(
    Math.abs(rb.x - ra.x) < 0.01,
    "b is left-aligned with the line, got " + rb.x + " vs " + ra.x,
  );
  check(
    rc.y > rb.y + 10,
    "no leak: c starts a new line, not a stale merge, got " +
      rc.y +
      " vs " +
      rb.y,
  );
...
test("sameLine before a group places the group on the previous element\u2019s line", () => {
  ...
  check(
    Math.abs(rb.y - ra.y) < 0.01,
    "group content sits on the previous element\u2019s line, got " +
      rb.y +
      " vs " +
      ra.y,
  );
  check(
    Math.abs(rb.x - (ra.x + ra.w + sp[0])) < 0.01,
    "group starts right of the previous element + spacing, got " +
      rb.x +
      " (expected " +
      (ra.x + ra.w + sp[0]) +
      ")",
...
test("sameLine after a group places the next element to the right of the group", () => {
  ...
  e.frame(draw);
  check(
    Math.abs(rb.y - rg.y) < 0.01,
    "text is on the group\u2019s line, got " + rb.y + " vs " + rg.y,
  );
  check(
    Math.abs(rb.x - (rg.x + rg.w + 16)) < 0.01,
    "text is 16px right of the group, got " +
      rb.x +
      " (group right edge " +
      (rg.x + rg.w) +
      ")",
...
test("a full-width item before a group does not inflate the group width", () => {
  ...
  e.frame(draw);
  check(
    Math.abs(g.w - (ra.x + e.gui._measure("short").w - g.x)) < 1,
    "group width = its own content, not the full line, got " + g.w,
  );
  check(
    Math.abs(rb.x - (g.x + g.w + 16)) < 0.01,
    "next element 16px right of the group, got " +
      rb.x +
      " (group right " +
      (g.x + g.w) +
      ")",
  ...
  check(
    Math.abs(rb.y - g.y) < 0.01,
    "next element on the group\u2019s line, got " + rb.y + " vs " + g.y,
  );
});
...
test("sameLine works between real widgets (combo + small button)", () => {
  ...
  check(
    Math.abs(r2.y - r1.y) < 0.01,
    "button on the same y level as the combo, got " + r1.y + " vs " + r2.y,
  );
  check(
    r2.x >= r1.x + r1.w,
    "button to the right of the combo, got " + r2.x + " vs " + (r1.x + r1.w),
  );
});
...
test("rapidly changing values do not desync", () => {
  ...
  check(
    ...
    "value sane after chaotic drag, got " + value,
  );
});
...
test("NaN / infinite values are handled", () => {
  ...
  check(
    isFinite(v2) || v2 === Infinity,
    "infinity input does not crash, got " + v2,
  );
});
...
test("debug overlay renders stats", () => {
  ...
  e.frames(5, draw);
  check(
    e.renderer.drawnText().some((t) => /FPS/.test(t)),
...
test("scrolled-out content is culled; no zero-size draws reach renderer", () => {
  ...
  check(
    w.maxScrollY > 100,
    "content is scrollable, maxScrollY=" + w.maxScrollY,
  );
  ...
  const bad = e.renderer.calls.filter((c) => {
    if (c[0] === "fillRect" || c[0] === "fillRoundedRect")
      return c[3] <= 0 || c[4] <= 0;
    return false;
  });
...
test("image widget forwards to renderer.drawImage", () => {
  ...
  e.frame(draw);
  check(
    e.renderer.calls.some((c) => c[0] === "drawImage" && c[1] === "tex1"),
    "drawImage called with id",
  );
});
...
test("window: title click collapses, outside-x click does not, drag does not", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("CC", { size: [260, 160] })) e.gui.endWindow();
  };
  e.frame(draw);
  ...
  check(
    w.collapsed === true,
    "title click collapsed, collapsed=" + w.collapsed,
  );
  e.click(tx, ty);
...
test("window: resize grip drawn at bottom-right, absent when fixed-size", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("GR", { size: [260, 160] })) e.gui.endWindow();
  };
  e.frame(draw);
  ...
  const grip = e.renderer.calls.filter(
    (c) => c[0] === "line" && nearCorner(c, gx, gy),
  );
  check(
    grip.length >= 2,
    "grip lines near bottom-right corner, got " + grip.length,
  );
  // the grip is a sizeable triangle: at least one endpoint well clear of the corner
  const d0 = (x, y) => Math.hypot(x - gx, y - gy);
  const reachesFar = grip.some(
    (c) => d0(c[1], c[2]) > 12 || d0(c[3], c[4]) > 12,
  );
  ...
  const draw2 = () => {
    if (
      e2.gui.beginWindow("FX", {
        size: [260, 160],
        flags: Mim.WindowFlags.FixedSize,
      })
    )
      e2.gui.endWindow();
  };
  ...
  const grip2 = e2.renderer.calls.filter(
    (c) => c[0] === "line" && nearCorner(c, w2.x + w2.w, w2.y + w2.h),
  );
  check(
    grip2.length === 0,
    "no grip on fixed-size window, got " + grip2.length,
  );
});
...
test("window: resize grip takes click priority over the scrollbar and highlights on hover", () => {
  ...
  const draw = () => {
    if (e.gui.beginWindow("SC", { size: [240, 170], pos: [100, 100] })) {
      for (let i = 0; i < 40; i++)
        e.gui.text("line " + i + " to overflow the window");
      e.gui.endWindow();
    }
  };
  e.frames(2, draw);
  ...
  const gripLine = (x, y) => {
    e.hover(x, y);
    e.frame(draw);
    const g = e.renderer.calls.filter(
      (c) =>
        c[0] === "line" &&
        Math.abs(c[1] - gx) < 20 &&
        Math.abs(c[2] - gy) < 20 &&
  ...
  check(
    ...
    "grip zone cursor, got " + e.renderer.lastCursor,
  );
  ...
  check(
    e.gui.state.drag &&
      ...
      e.gui.state.drag.edge === 3,
    "corner click starts a resize, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  const w0w = e.gui.getWindow("SC").w,
    w0h = e.gui.getWindow("SC").h; // snapshot!
  e.dragTo(gx - 4, gy - 4, gx + 30, gy + 20);
  const w1 = e.gui.getWindow("SC");
  check(
    w1.w > w0w + 20 && w1.h > w0h + 10,
    "grip drag resized the window, got " + w1.w + "x" + w1.h,
  );
  // the scrollbar still works away from the grip zone (window moved: re-aim)
  e.press(w1.x + w1.w - 4, w1.y + w1.h / 2);
  check(
    e.gui.state.drag && e.gui.state.drag.type === "scroll-v",
    "mid-track click still drags the scrollbar, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.up(0);
  e.frame(draw);
...
test("window: resize bars pop up near a side, drag resizes that direction, N configurable", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("RB", { size: [240, 200], pos: [100, 100] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  check(
    Number.isInteger(e.gui.flags.resizeBarProximity) &&
  ...
  e.hover(w.x - 3, w.y + 100);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "ew-resize",
    "left side shows ew-resize, got " + e.renderer.lastCursor,
  );
  ...
  check(
    bar.length >= 1,
    "left resize bar drawn over the left side, got " + bar.length,
  );
  const x0 = w.x,
    w0 = w.w; // snapshots (windows are live objects)
  e.dragTo(w.x - 3, w.y + 100, w.x - 40, w.y + 100);
  const w2 = e.gui.getWindow("RB");
  check(
    Math.abs(w2.x - (x0 - 37)) < 1 && Math.abs(w2.w - (w0 + 37)) < 1,
  ...
  check(
    Math.abs(w3.y - (y0 - 27)) < 1 && Math.abs(w3.h - (h0 + 27)) < 1,
    "top drag moved y and grew h, got y=" + w3.y + " h=" + w3.h,
  );
  // CORNER: both bars + grip, drag resizes both directions at once
  e.hover(w3.x + w3.w + 4, w3.y + w3.h + 4);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "nwse-resize",
    "corner shows nwse-resize, got " + e.renderer.lastCursor,
  );
  const c0w = w3.w,
    c0h = w3.h; // snapshot before the corner drag
  e.dragTo(
    w3.x + w3.w + 4,
    w3.y + w3.h + 4,
    w3.x + w3.w + 24,
  ...
  check(
    w4.w > c0w + 10 && w4.h > c0h + 5,
    "corner drag resized both dimensions at once, got " +
      w4.w +
      "x" +
      w4.h +
      " (was " +
  ...
  e.gui.flags.resizeBarProximity = 0;
  e.hover(w4.x + w4.w + 3, w4.y + 50);
  e.frame(draw);
  check(
    e.renderer.lastCursor !== "ew-resize",
    "proximity 0 disables the side bands",
  );
  e.gui.flags.resizeBarProximity = 3;
  e.hover(w4.x + w4.w + 4, w4.y + 50);
  e.frame(draw);
  check(e.renderer.lastCursor !== "ew-resize", "4px out is outside N=3");
  e.hover(w4.x + w4.w + 2, w4.y + 50);
  e.frame(draw);
...
test("listBox: selects on click, shows highlight, scrolls selection into view", () => {
  ...
  const draw = () => {
    if (e.gui.beginWindow("LB", { size: [280, 320] })) {
      ...
      for (const w of e.gui.state.windows.values())
        if (w.kind === "child") child = w;
      e.gui.endWindow();
    }
  };
  e.frame(draw);
  ...
  check(
    child.scrollY > 0,
    "scrolled to selected row, scrollY=" + child.scrollY,
  );
...
test("cursor: pointer over button, move over title, text in edit, grabbing while dragging", () => {
  ...
  // over the button
  e.hover(btn.x + 5, btn.y + btn.h / 2);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "pointer",
    "pointer over button, got " + e.renderer.lastCursor,
  );
  // over the title bar
  e.hover(w.x + w.w / 2, w.y + w.titleH / 2);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "move",
    "move over title, got " + e.renderer.lastCursor,
  );
  // editing the text field
  e.click(fld.x + 10, fld.y + fld.h / 2);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "text",
    "text cursor while editing, got " + e.renderer.lastCursor,
  );
  // dragging the window
  e.press(w.x + w.w / 2, w.y + w.titleH / 2);
  e.hover(w.x + w.w / 2 + 30, w.y + w.titleH / 2 + 20);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "grabbing",
    "grabbing while dragging, got " + e.renderer.lastCursor,
  );
  e.release();
  e.frame(draw);
});
...
test("cursor: not requested when backend lacks the feature", () => {
  ...
  e.frame(draw);
  check(
    e.renderer.lastCursor === "default",
    "no setCursor when unsupported, got " + e.renderer.lastCursor,
  );
});
...
test("tooltip: only topmost hovered item shows it, above the cursor after delay", () => {
  ...
  const texts = e.renderer.drawnText();
  check(
    texts.some((t) => t === "top tooltip"),
    "topmost tooltip shown",
  );
  check(!texts.some((t) => t === "bot tooltip"), "occluded tooltip suppressed");
  // tooltip box is drawn above the cursor
  const dt = e.renderer.calls.find(
    (c) => c[0] === "drawText" && c[3] === "top tooltip",
  );
  check(
    dt && dt[2] < e.input.mouse.y,
    "tooltip positioned above cursor (y=" +
      (dt && dt[2]) +
      " < " +
      e.input.mouse.y +
      ")",
...
test("dock: joins two windows, divider drag, move, resize, undock, close", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("A", { size: [240, 200], pos: [100, 100] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { size: [240, 200], pos: [400, 100] }))
      e.gui.endWindow();
  };
  e.frame(draw);
  ...
  let A = e.gui.getWindow("A"),
    B = e.gui.getWindow("B");
  check(
    Math.abs(A.x + A.w + 6 - B.x) < 2 && A.y === B.y && A.h === B.h,
    "horizontal dock: A left of B sharing height, A right=" +
      (A.x + A.w) +
      " B x=" +
      B.x,
  );
  ...
  A = e.gui.getWindow("A");
  check(
    Math.abs(A.x - (x0 + 120)) < 2,
    "dock move translated member A, got dx=" + (A.x - x0),
  );
  ...
  check(
    e.gui.getDocks()[0].w > w0 + 60,
    "dock resize widened it, got +(" + (e.gui.getDocks()[0].w - w0) + ")",
  );
...
test("dock: close button removes dock and closes both members; vertical dock works", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("P", { size: [200, 160], pos: [120, 120] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("Q", { size: [200, 160], pos: [120, 340] }))
      e.gui.endWindow();
  };
  e.frame(draw);
  e.gui.dock("P", "Q", { dir: "v" });
  e.frame(draw);
  let P = e.gui.getWindow("P"),
    Q = e.gui.getWindow("Q");
  check(
    Math.abs(P.y + P.h + 6 - Q.y) < 2 && P.x === Q.x,
    "vertical dock: P above Q",
  );
  ...
  check(
    e.gui.getWindow("P").open === false && e.gui.getWindow("Q").open === false,
    "both members closed",
  );
});
...
test("dock: slim header collapses a member (other grows); undock button frees one member", () => {
  ...
  B = e.gui.getWindow("B");
  check(
    A.collapsed === true && B.collapsed === false,
    "only the clicked member collapsed",
  );
...
test("app menu bar: top bar, section open, item click, keyboard shortcut, activateMenu", () => {
  ...
  e.gui.setAppMenuBar(
    [
      ...
      {
        label: "Windows",
        items: [{ label: "Toggle A", onActivated: () => toggled++ }],
      },
    ],
    { pos: "top" },
  );
  const draw = () => {
    if (e.gui.beginWindow("M", { size: [200, 140], pos: [100, 80] }))
      e.gui.endWindow();
  };
  e.frame(draw);
  check(
    e.gui.state.appBarRect &&
      e.gui.state.appBarRect.w === 1280 &&
      e.gui.state.appBarRect.h === 30,
  ...
  check(
    pops.length === 1 && pops[0].data.appMenu,
    "section click opened dropdown",
  );
  const fp = pops[0];
  check(
    fp.data.items.length === 3 && fp.data.items[1].type === "sep",
    "dropdown rows include sep",
  );
  // click the 'Save' row
  const rowH = e.gui._lineH() + 10;
  ...
  check(
    !e.gui.state.popupList.some((p) => p.open),
  ...
  // activateMenu
  check(
    e.gui.activateMenu("Windows", "Toggle A") === true && toggled === 1,
    "activateMenu triggers item",
  );
  check(
    e.gui.activateMenu("Windows", "Nope") === false,
    "activateMenu misses unknown item",
  );
});
...
test("app menu bar: submenu hover-open, and bar blocks clicks to windows underneath", () => {
    ...
    [
      {
        ...
        items: [
          {
            label: "Sub",
            items: [{ label: "Deep", onActivated: () => deep++ }],
          },
        ],
      },
    ],
  ...
  e.frame(draw);
  check(
    e.gui.state.appBarRect.x === 0 && e.gui.state.appBarRect.w === 180,
    "left bar geometry",
  );
  ...
  e.frame(draw);
  const sp = e.gui.state.popupList.find(
    (p) => p.open && p.id.startsWith("##appsub"),
  );
  check(
    !!sp && sp.data.items.length === 1,
    "submenu opened on hover with its rows",
  );
  ...
  check(
    under === 0 || under === 0,
    "no stray clicks elsewhere (button not under cursor)",
  );
});
...
function twoWinEnv() {
  ...
  const draw = () => {
    if (e.gui.beginWindow("A", { size: [240, 200], pos: [100, 100] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { size: [240, 200], pos: [500, 100] }))
      e.gui.endWindow();
  };
  e.frame(draw);
  return { e, draw };
...
test("drag grid: drop on a part of the hovered window docks (side sets dir/order)", () => {
  ...
  check(
    D && D.x === 500 && D.y === 100 && D.w === 240 && D.h === 400,
    "v-dock height is the sum of both windows, got " +
      (D && JSON.stringify({ x: D.x, y: D.y, w: D.w, h: D.h })),
  );
  const A = e.gui.getWindow("A"),
    B = e.gui.getWindow("B");
  check(
    A.y < B.y && Math.abs(A.y + A.h + 6 - B.y) < 2,
    "A above B sharing height",
  );
  ...
  const drawC = () => {
    draw();
    if (e.gui.beginWindow("C", { size: [200, 160], pos: [100, 400] }))
      e.gui.endWindow();
  };
  e.clearDraw();
  e._draw = null;
  ...
  e.hover(647, 200);
  e.frame(drawC); // B's right triangle (keep C open!)
  e.release();
  check(e.gui.isDocked("B", "C"), "docked after drop on right part");
  D = e.gui.getDocks()[0];
  check(
    D && D.dir === "h" && D.a === "B",
    "horizontal dock, target is left member",
  );
  ...
  e.press(220, 114);
  e.hover(620, 200);
  e.frame(draw); // exactly B's center
  check(
    e.gui.state._dockHint && e.gui.state._dockHint.side === null,
    "center apex has no direction",
  );
  e.release();
  check(!e.gui.isDocked("A", "B"), "center apex drop docks nothing");
  check(
    e.gui.getWindow("A").open === true,
    "window still open after center drop",
  );
});
...
test("drag grid: NoDock denies docking (as target and as source); flags.docking gates it", () => {
  ...
  check(
    !e.gui.state._dockHint || e.gui.state._dockHint.kind !== "window",
    "no grid over a NoDock target",
  );
  ...
  const e2 = new Env();
  const draw2 = () => {
    if (e2.gui.beginWindow("P", { size: [240, 200], pos: [100, 100] }))
      e2.gui.endWindow();
    if (e2.gui.beginWindow("Q", { size: [240, 200], pos: [500, 100] }))
      e2.gui.endWindow();
  };
...
test("screen edges: drag to an edge band docks the window to the screen side", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("W", { size: [240, 200], pos: [100, 100] }))
      e.gui.endWindow();
  };
  e.frame(draw);
  ...
  check(
    W.x === 6 && W.w === 228,
    "laid out in the column, got x=" + W.x + " w=" + W.w,
  );
  check(
    W.movable === false && W.resizable === false,
    "edge window is not movable/resizable",
  );
  // a second window joins the same edge (stacked vertically by default)
  const draw2 = () => {
    draw();
    if (e.gui.beginWindow("V", { size: [200, 160], pos: [400, 400] }))
      e.gui.endWindow();
  };
  e.frame(draw2);
  ...
  check(
    V._edge === "left" && e.gui.state.edgeDocks.left.wins.length === 2,
    "both in the left stack",
  );
  check(
    Math.abs(V.h - W.h) < 2,
    "equal vertical shares, got " + W.h + " vs " + V.h,
  );
  ...
  check(
    W.h < V.h,
    "boundary drag grew the lower window, got " + W.h + " vs " + V.h,
  );
  ...
  check(
    e.gui.state.edgeDocks.left.size > w0 + 20,
    "inner-edge drag widened column, +" +
      (e.gui.state.edgeDocks.left.size - w0),
  );
  // double-click a title bar frees the window from the edge
  e.click(V.x + 100, V.y + 14);
  e.click(V.x + 100, V.y + 14);
  check(
    !V._edge && V.movable === true,
    "double-click undocked from edge, movable restored",
  );
  check(
    e.gui.state.edgeDocks.left.wins.length === 1,
    "only W remains in the stack",
  );
...
test("screen edges: right/top edges + top band respects app menu bar", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("W", { size: [200, 120], pos: [100, 100] }))
      e.gui.endWindow();
  };
  e.gui.setAppMenuBar(
    [{ label: "F", items: [{ label: "x", onActivated: () => {} }] }],
    { pos: "top" },
  );
  e.frame(draw);
  ...
  check(
    W.y === 36,
    "top row sits below the top app bar with padding (y=36), got " + W.y,
  );
  check(W.x === 6, "row inset from left");
  e.gui.undockEdge("W");
  e.frame(draw);
  check(
    !W._edge && e.gui.state.edgeDocks.top === null,
    "undockEdge frees and dissolves",
  );
});
...
test("app menu bar: click on the bar never collapses a window underneath", () => {
  ...
  check(
    !!e.gui.state.popupList.find((p) => p.open && p.data.appMenu),
    "bar section still opens",
  );
  // an outside click dismisses the popup
  e.click(700, 300);
  check(
    !e.gui.state.popupList.find((p) => p.open && p.data.appMenu),
    "outside click closed the menu",
  );
  ...
  check(
    T.collapsed === true,
    "popup click did not flip the window collapse again",
  );
});
...
test("dock chevron: collapses/expands the whole dock (members hidden, header only)", () => {
  ...
  e.frame(draw);
  check(
    e.gui.getWindow("A").w === 0 && e.gui.getWindow("B").w === 0,
    "members hidden while dock collapsed",
  );
  check(
    e.gui.isDockCollapsed("A", "B") && live().collapsed,
  ...
  e.frame(draw);
  check(
    !e.gui.isDockCollapsed("A", "B") && e.gui.getWindow("A").w > 100,
    "setDockCollapsed restores members",
  );
  // chevron again
  e.click(D.x + 14, D.y + tH / 2);
  ...
  check(
    live().x > x0 + 40,
    "collapsed dock still draggable by title, dx=" + (live().x - x0),
  );
});
...
test("collapsed window: header only (no body, no shadow, hover = header row)", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("Top", { size: [300, 300], pos: [100, 100] }))
      e.gui.endWindow();
  };
  e.frame(draw);
  ...
  const body = fills.find(
    (c) =>
      isBg(c) &&
      Math.abs(c[1] - T.x) < 0.01 &&
      Math.abs(c[2] - T.y) < 0.01 &&
      Math.abs(c[3] - T.w) < 0.01,
  );
  check(
    !!body && Math.abs(body[4] - tH) < 0.01,
    "collapsed body is header height only, got h=" + (body && body[4]),
  );
  const shadows = fills.filter(
    (c) =>
      c[5] && c[5][0] === 0 && c[5][1] === 0 && c[5][2] === 0 && c[5][3] < 255,
  );
  ...
  e.frame(draw);
  check(
    e.gui.state.hoveredWindow !== T,
    "body area no longer hovered while collapsed",
  );
  e.hover(150, 100 + tH / 2);
  e.frame(draw);
  ...
  check(
    !!body2 && Math.abs(body2[4] - 300) < 0.01,
    "expanded body back to full height, got h=" + (body2 && body2[4]),
  );
});
