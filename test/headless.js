...
test('tab content keeps its size and scrolls 1:1 when the window scrolls', () => {
  ...
  const draw = () => {
    ...
    if (e.gui.beginWindow('TBSC', { size: [300, 160] })) {
      ...
      e.gui.endTabItem();
      if (e.gui.beginTabItem('Two')) { e.gui.text('two'); }
      e.gui.endTabItem();
      e.gui.endTabBar();
      e.gui.endWindow();
    }
  ...
  check(approx(availTop, availBottom, 0.01),
    'tab region height unchanged by scroll (' + availTop.toFixed(1) + ' -> ' + availBottom.toFixed(1) + ')');
  check(approx(cursorTop - cursorBottom, win.scrollY, 0.01),
    'tab content scrolls 1:1 with the window (moved ' + (cursorTop - cursorBottom).toFixed(1) +
    ' for scroll ' + win.scrollY.toFixed(1) + ')');
  // and back to the top: everything is exactly as before
  win.scrollY = win.scrollTargetY = 0;
  e.frame(draw);
  check(approx(availAtTabStart, availTop, 0.01), 'back at top: tab region height restored');
});
