...
test('pushId disambiguates repeated widgets in a loop', () => {
    ...
    e.frame(draw);
    check(
        rects.length === 3 && rects[0].y !== rects[1].y && rects[1].y !== rects[2].y,
...
test('style push/pop overrides colors for one block', () => {
    ...
    const red = e.renderer.calls.find(
        (c) => c[0] === 'fillRoundedRect' && c[6] && c[6][0] === 200 && c[6][1] === 30,
    );
...
test('setNextItemWidth constrains slider width', () => {
    ...
    check(
        r1.w < r2.w,
        'next-item width applied only to the next widget (' + r1.w + ' vs ' + r2.w + ')',
    );
});
...
test('sameLine supports explicit spacing and an absolute line offset', () => {
    ...
    check(
        ...
        'offsetX anchors from the line start, got x ' + rd.x + ' (line start ' + rc.x + ')',
    );
});
...
test('sameLine chains across a row, then the next element drops below the row', () => {
    ...
    check(
        rd.y > rc.y + 10,
        'the element after the row starts a new line below it, got ' + rd.y + ' vs ' + rc.y,
    );
    check(
...
test('sameLine with no previous element is a no-op (if applicable)', () => {
    ...
    check(
        ...
        'the second element is left-aligned (not merged), got ' + r2.x + ' vs ' + r1.x,
    );
});
...
test('an empty group after sameLine does not leak the request', () => {
    ...
    check(
        rc.y > rb.y + 10,
        'no leak: c starts a new line, not a stale merge, got ' + rc.y + ' vs ' + rb.y,
    );
});
...
test('sameLine before a group places the group on the previous element\u2019s line', () => {
    ...
    check(
        ...
        'group content sits on the previous element\u2019s line, got ' + rb.y + ' vs ' + ra.y,
    );
    check(
...
test('a full-width item before a group does not inflate the group width', () => {
    ...
    check(
        ...
        'next element 16px right of the group, got ' + rb.x + ' (group right ' + (g.x + g.w) + ')',
    );
    check(
...
test('window: resize grip drawn at bottom-right, absent when fixed-size', () => {
    ...
    const e2 = new Env();
    const draw2 = () => {
        if (e2.gui.beginWindow('FX', { size: [260, 160], flags: Mim.WindowFlags.FixedSize }))
            e2.gui.endWindow();
    };
    e2.frame(draw2);
...
test('window: resize grip takes click priority over the scrollbar and highlights on hover', () => {
    ...
    e.press(gx - 4, gy - 4);
    check(
        e.gui.state.drag && e.gui.state.drag.type === 'win-resize' && e.gui.state.drag.edge === 3,
...
test('window: resize bars pop up near a side, drag resizes that direction, N configurable', () => {
    const e = new Env();
    const draw = () => {
        if (e.gui.beginWindow('RB', { size: [240, 200], pos: [100, 100] })) e.gui.endWindow();
    };
    e.frames(2, draw);
    check(
        Number.isInteger(e.gui.flags.resizeBarProximity) && e.gui.flags.resizeBarProximity > 0,
...
test('tooltip: only topmost hovered item shows it, above the cursor after delay', () => {
    ...
    check(
        ...
        'tooltip positioned above cursor (y=' + (dt && dt[2]) + ' < ' + e.input.mouse.y + ')',
    );
});
...
test('app menu bar: top bar, section open, item click, keyboard shortcut, activateMenu', () => {
    ...
    e.gui.setAppMenuBar(
        [
            ...
            { label: 'Windows', items: [{ label: 'Toggle A', onActivated: () => toggled++ }] },
        ],
        { pos: 'top' },
    );
    const draw = () => {
        if (e.gui.beginWindow('M', { size: [200, 140], pos: [100, 80] })) e.gui.endWindow();
    };
    e.frame(draw);
    check(
...
test('app menu bar: submenu hover-open, and bar blocks clicks to windows underneath', () => {
    ...
    e.gui.setAppMenuBar(
        [
            {
                label: 'W',
                items: [{ label: 'Sub', items: [{ label: 'Deep', onActivated: () => deep++ }] }],
            },
        ],
        { pos: 'left' },
    );
...
test('drag grid: NoDock denies docking (as target and as source); flags.docking gates it', () => {
    ...
    const draw = () => {
        if (e.gui.beginWindow('A', { size: [240, 200], pos: [100, 100] })) e.gui.endWindow();
        if (
            e.gui.beginWindow('N', { size: [240, 200], pos: [500, 100], flags: WindowFlags.NoDock })
        )
            e.gui.endWindow();
    };
    e.frame(draw);
...
test('screen edges: drag to an edge band docks the window to the screen side', () => {
    ...
    check(
        ...
        'inner-edge drag widened column, +' + (e.gui.state.edgeDocks.left.size - w0),
    );
...
test('screen edges: right/top edges + top band respects app menu bar', () => {
    ...
    e.gui.setAppMenuBar([{ label: 'F', items: [{ label: 'x', onActivated: () => {} }] }], {
        pos: 'top',
    });
    e.frame(draw);
...
test('collapsed window: header only (no body, no shadow, hover = header row)', () => {
    ...
    const isBg = (c) =>
        c[6] && c[6][0] === bg[0] && c[6][1] === bg[1] && c[6][2] === bg[2] && c[6][3] === bg[3];
    ...
    const shadows = fills.filter(
        (c) => c[5] && c[5][0] === 0 && c[5][1] === 0 && c[5][2] === 0 && c[5][3] < 255,
    );
