...
{
  const Ad = W('Addons');
  const inAd = (c) => c[2] >= Ad.x && c[2] <= Ad.x + Ad.w && c[3] >= Ad.y && c[3] <= Ad.y + Ad.h;
  let colorTab = null, widgetsTab = null;
  ...
  if (colorTab) {
    move(colorTab.x, colorTab.y); frame();
    down(colorTab.x, colorTab.y, 0); frame();
    up(colorTab.x, colorTab.y, 0); frame(); frame();
  }
  const acc = gui._state('##colorbtnaccent');
  check(!!acc && Array.isArray(acc.hsv), 'Color tab renders the alpha color button');
  if (acc) {
    acc.open = true;
    frame(); frame();
    check(calls.some((c) => c[0] === 'ctx.fillText' && c[1] === 'alpha'), 'alpha slider row rendered');
  }
  if (widgetsTab) {
    move(widgetsTab.x, widgetsTab.y); frame();
    down(widgetsTab.x, widgetsTab.y, 0); frame();
    up(widgetsTab.x, widgetsTab.y, 0); frame(); frame();
  }
  check(calls.some((c) => c[0] === 'ctx.fillText' && c[1] === 'volume'), 'Widgets tab renders the grabber');
  const bolds = calls.filter((c) => c[0] === 'ctx.fillText' && c[1] === 'bold');
  check(bolds.length >= 2, 'markdown bold is double-stroked, got ' + bolds.length + ' passes');
}
