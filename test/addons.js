...
console.log("• addon registry: register/uninstall/opt-in");
{
  ...
  check(
    !!e.gui.addons.plots && !e.gui.addons.t3d,
    "subset install: plots only",
  );
  // all off
  ...
  check(
    !global.Mim.addonNames().includes("myaddon"),
    "unregister removes addon",
  );
}
...
console.log(
  "• plots: bezier draggable (normalized points), polar + heatmap draw",
);
{
  const e = new Env();
  let size = [420, 300];
  ...
  check(
    !!st && st.ptsN && st.ptsN.length === 4,
    "bezier default 4 control points (normalized)",
  );
  check(
    Array.isArray(curve) && curve.length === 4,
    "plotBezier returns point list",
  );
  ...
  const draw2 = () => {
    if (e2.gui.beginWindow("P2", { size: [420, 560], pos: [40, 40] })) {
      e2.gui.addons.plots.plotPolar(
        "rose",
        (t) => 0.5 + 0.5 * Math.sin(4 * t),
        { h: 200 },
      );
...
console.log("• 3d: surface draws, drag rotates, wheel zooms");
{
  ...
  check(
    lines.length > 100,
    "surface wireframe draws many lines, got " + lines.length,
  );
}
...
console.log("• tables: sort, select, column resize");
{
  ...
  e.frame(draw);
  check(
    st.sortCol === "name" && st.sortDir === 1,
    "header click sorts ascending",
  );
  check(
    res.sorted[0].name === "apple" && res.sorted[2].name === "cherry",
    "rows sorted by name",
  );
  // click again -> desc
  e.click(box.x + 60, box.y + 14);
  e.frame(draw);
  check(
    st.sortDir === -1 && res.sorted[0].name === "cherry",
  ...
  e.frame(draw);
  check(
    st.selected.length === 2,
    "ctrl+click multi-selects, got " + JSON.stringify(st.selected),
  );
...
console.log("• scaling: share/fill heights + widths follow the window");
{
  ...
  check(
    bars.w === 200,
    "plot width follows window width (no overflow scrollbar), got " + bars.w,
  );
  check(
    bars.h < h0 && bars.h > 48,
    "share height follows window height, got " +
      Math.round(bars.h) +
      " from " +
      Math.round(h0),
  );
  ...
  check(
    big && big.h > 250,
    "no-share core plotLines fills the available height, got " +
      (big && Math.round(big.h)),
  );
}

console.log(
  "• scrolling: plots in a tab bar keep their size while the window scrolls",
);
{
  const e = new Env();
  let topBox = null,
  ...
  const draw = () => {
    if (e.gui.beginWindow("PT", { size: [300, 190], pos: [40, 40] })) {
      ...
      if (e.gui.beginTabItem("Curves")) {
        ...
        if (b) {
          if (e.gui.state.windows.get("PT").scrollY > 0)
            bottomBox = { h: b.h, y: b.y };
          else if (topBox === null) topBox = { h: b.h, y: b.y };
        }
      }
      e.gui.endTabItem();
      if (e.gui.beginTabItem("Other")) {
        e.gui.text("x");
      }
      e.gui.endTabItem();
      e.gui.endTabBar();
  ...
  const hBottom = bottomBox.h,
    yBottom = bottomBox.y;
  check(
    Math.abs(hTop - hBottom) < 0.01,
    "shared plot height unchanged by scroll (" +
      hTop.toFixed(1) +
      " -> " +
      hBottom.toFixed(1) +
      ")",
  ...
  check(
    Math.abs(yTop - yBottom - win.scrollY) < 0.01,
    "plot top edge scrolls 1:1 with the window (moved " +
      (yTop - yBottom).toFixed(1) +
      " for scroll " +
...
console.log("• color picker auto-sizes to its content (no scrollbar)");
{
  ...
  for (const cfg of configs) {
    ...
    e.frame(draw);
    let child = null;
    for (const [, w] of e.gui.state.windows)
      if (w.kind === "child") {
        child = w;
        break;
      }
    ...
    check(
      child && child.maxScrollY <= 0.001,
      cfg.label +
        ": picker has no scrollbar (maxScrollY=" +
        (child && child.maxScrollY.toFixed(1)) +
        ")",
    );
    check(
      child && child.contentH + child.padY <= child.visibleContentH + 0.01,
      cfg.label + ": picker content fits the child frame",
    );
    const texts = e.renderer.drawnText();
    check(
      cfg.opts.alpha ? texts.includes("alpha") : !texts.includes("alpha"),
...
console.log("• plots: bars (negatives, hover, NaN) + multi-series legend/gaps");
{
  ...
  const draw = () => {
    if (e.gui.beginWindow("P", { size: [420, 360], pos: [40, 40] })) {
      barBox = e.gui.addons.plots.plotBars("bars", [10, -4, NaN, 22], {
        h: 100,
      });
  ...
  check(
    fills.length >= 4,
    "bars draw one rect per value (NaN dropped), got " + fills.length,
  );
  check(
    e.renderer.drawnText().includes("a") &&
      e.renderer.drawnText().includes("b"),
...
console.log("• color: colorButton picker (swatch, hsv, hex, presets)");
{
  ...
  check(
    JSON.stringify(C.norm("#2b4a8b")) === "[43,74,139,255]",
    "norm hex -> rgba",
  );
  check(C.toHex([43, 74, 139]) === "#2b4a8b", "toHex round trip");
  check(
    C.fromHex("zz") == null && C.fromHex("#abc") != null,
    "fromHex validates input",
  );
  let sw = null;
  ...
  e.frame(draw);
  check(
    e.gui._state("##colorbtncol").open === true,
    "clicking the swatch opens the picker",
  );
  const svPad = e.renderer.calls.find(
    (c) => c[0] === "fillRect" && c[4] === 110 && c[3] > 100,
  );
  const hueStrip = e.renderer.calls.find(
    (c) => c[0] === "fillRect" && c[4] === 12,
  );
  check(
    !!svPad && !!hueStrip && e.renderer.drawnText().includes("hex"),
    "picker shows SV pad + hue bar + hex field",
  );
  ...
  const hexLabel = e.renderer.calls.find(
    (c) => c[0] === "drawText" && c[3] === "hex",
  );
...
console.log("• notifications: toasts stack top-right and expire after ttl");
{
  ...
  check(
    NF.count === 2 && rects.length === 2,
    "two toasts drawn, got " + rects.length,
  );
  check(
...
console.log("• tables: null-safe sort, missing cells, id-based selection");
{
  ...
  check(
    ...
    "missing/null values sort last, got " +
      JSON.stringify(res.sorted.map((r) => r.name)),
  );
  check(
    e.renderer.drawnText().includes("\u2014"),
    "missing cell renders an em dash",
  );
  ...
  check(
    selCount === 1,
    "row click selects one row (id key), got " + JSON.stringify(st.selected),
  );
  e.click(box.x + 130, box.y + 14); // sort by size
  e.frame(draw);
  check(
    st.selected.length === selCount,
    "selection survives re-sorting (id-based)",
  );
}
...
console.log("• 3d: spin option + points sanitized");
{
  ...
  check(
    ...
    "spin rotates the view over time, got " +
      st.rotY.toFixed(3) +
      " from " +
      r0.toFixed(3),
  );
  ...
  check(
    circles === 1,
    "non-finite/missing points are dropped, got " + circles + " dots",
  );
  const e3 = new Env();
  const draw3 = () => {
  ...
  e3.frame(draw3);
  check(
    e3.renderer.drawnText().includes("no points"),
    "empty point set shows a placeholder",
  );
}
...
console.log("• color: SV pad, hue bar, alpha, checkerboard");
{
  ...
  const draw = () => {
    if (e.gui.beginWindow("CP", { size: [340, 420], pos: [40, 40] })) {
      col = e.gui.addons.color.colorButton("c2", col, {
        alpha: true,
        presets: false,
      });
      e.gui.endWindow();
  ...
  for (const c of e.renderer.calls) {
    ...
    if (c[4] === 110 && c[3] > 100 && (!pad || c[3] > pad.w))
      pad = { x: c[1], y: c[2], w: c[3], h: 110 };
  ...
  check(
    hsvAfter[1] > 0.7,
    "pad drag raised saturation, got " + hsvAfter[1].toFixed(2),
  );
  check(
    hsvAfter[2] > 0.8,
    "pad drag raised value, got " + hsvAfter[2].toFixed(2),
  );
  ...
  const checker = e.renderer.calls.filter(
    (c) => c[0] === "fillRect" && c[3] === 7 && c[4] === 7,
  ).length;
  check(
    checker >= 4,
    "checkerboard drawn for transparent swatch, got " + checker + " squares",
  );
...
console.log("• plots: grid toggle, axis labels, marker control");
{
  ...
  const draw = () => {
    if (e.gui.beginWindow("P2", { size: [360, 300], pos: [40, 40] })) {
      e.gui.addons.plots.plotSeries("s2", [{ name: "a", values: vals }], {
        h: 140,
        axisLabels: true,
      });
      e.gui.endWindow();
  ...
  const texts = e.renderer.drawnText();
  check(
    texts.includes("4.0") && texts.includes("1.0"),
    "axis labels show min/max (4.0 / 1.0)",
  );
  ...
  const draw2 = () => {
    if (e2.gui.beginWindow("P3", { size: [360, 300], pos: [40, 40] })) {
      e2.gui.addons.plots.plotSeries("s3", [{ name: "a", values: longVals }], {
        h: 140,
        grid: false,
        marker: false,
      });
  ...
  check(
    gridOffLines === gridLines - 8,
    "grid: false removes the 8 grid lines (" +
      gridOffLines +
      " vs " +
      gridLines +
      ")",
  ...
  check(
    e2.renderer.calls.filter((c) => c[0] === "fillCircle").length === 0,
    "marker: false removes sample dots",
  );
  // marker: true forces dots even on long series
  const e3 = new Env();
  const draw3 = () => {
    if (e3.gui.beginWindow("P4", { size: [360, 300], pos: [40, 40] })) {
      e3.gui.addons.plots.plotSeries("s4", [{ name: "a", values: longVals }], {
        h: 140,
        grid: false,
        marker: true,
      });
  ...
  e3.frame(draw3);
  check(
    e3.renderer.calls.filter((c) => c[0] === "fillCircle").length ===
      longVals.length,
    "marker: true forces dots on every sample",
  );
}
...
console.log("• tables: filter row");
{
  ...
  const draw = () => {
    if (e.gui.beginWindow("T2", { size: [360, 340], pos: [40, 40] })) {
      shown = e.gui.addons.tables.advancedTable(
        "t2",
        [
          { id: "name", label: "Name", width: 140 },
          { id: "val", label: "Val", width: 60 },
        ],
        rows,
        { h: 150, filter: true },
      );
  ...
  e.frame(draw);
  check(
    shown.shown === 1 && shown.total === 3,
    'filter "be" shows only beta, got ' + shown.shown + " of " + shown.total,
  );
  check(
    e.renderer.drawnText().includes("beta") &&
      !e.renderer.drawnText().includes("alpha"),
    "beta visible, alpha hidden",
  );
  ...
  e.click(frame[1] + frame[3] / 2, frame[2] + frame[4] / 2);
  for (const st2 of e.gui.state.widgetStates.values()) {
    if (st2 && st2.edit === true && typeof st2.buf === "string") {
      st2.buf = "zzz";
      st2.caret = 3;
    }
  }
  ...
  e.frame(draw);
  check(
    shown.shown === 0 && e.renderer.drawnText().includes("no matching rows"),
    "no match -> placeholder, shown=" + shown.shown,
  );
}
...
console.log("• widgets: grabber drag, spinner, bullet");
{
  ...
  check(
    ...
    "spinner draws its arc",
  );
  check(
    e.renderer.calls.some((c) => c[0] === "fillCircle"),
    "bullet draws its circle",
  );
  check(e.renderer.drawnText().includes("point one"), "bullet draws its text");
  // drag the knob to the right end
  const knob = e.renderer.calls.find(
    (c) => c[0] === "fillRoundedRect" && c[3] === 10 && c[4] === 16,
  );
  const track = e.renderer.calls.find(
    (c) => c[0] === "fillRoundedRect" && c[4] === 6 && c[3] > 200,
  );
...
console.log("• markdown: styles, code chip, wrapping");
{
  ...
  const draw = () => {
    if (e.gui.beginWindow("MD", { size: [300, 300], pos: [40, 40] })) {
      lines = e.gui.addons.markdown.md(
        "**bold** *italic* `code` and a much longer line that should wrap around nicely to the next line",
        { w: 240 },
      );
      e.gui.endWindow();
    }
  ...
  const texts = e.renderer.calls
    .filter((c) => c[0] === "drawText")
    .map((c) => String(c[3]));
  check(
    texts.filter((t) => t === "bold").length === 2,
    "bold drawn double-stroked (fake bold), got " +
      texts.filter((t) => t === "bold").length,
  );
  const codeChip = e.renderer.calls.find(
    (c) => c[0] === "fillRect" && c[4] > 10 && c[3] < 40,
  );
  ...
  const runs = e.gui.addons.markdown
    .parseLine("**a** *b* `c` d")
    .filter((r) => r.text.trim() || r.style !== "plain");
  check(
    runs.length === 4 &&
      runs[0].style === "bold" &&
      runs[1].style === "italic" &&
      runs[2].style === "code" &&
      runs[3].style === "plain",
    "parseLine extracts bold/italic/code/plain runs, got " +
