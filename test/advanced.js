/* Advanced feature tests: header context menus, covered-element hover gating,
...
class MockRenderer {
  ...
  setLayer(l) {
    this.layer = l;
    this.record("setLayer", [l]);
  }
  setCursor(c) {
    this.lastCursor = c;
    this.record("setCursor", [c]);
  }
  pushClip(x, y, w, h) {
    const c = this.clip;
  ...
  fillRect(x, y, w, h, c) {
    this.record("fillRect", [x, y, w, h, c]);
  }
  fillRoundedRect(x, y, w, h, r, c) {
    this.record("fillRoundedRect", [x, y, w, h, r, c]);
  }
  strokeRect(x, y, w, h, c, t) {
    this.record("strokeRect", [x, y, w, h, c, t]);
  }
  strokeRoundedRect(x, y, w, h, r, c, t) {
    this.record("strokeRoundedRect", [x, y, w, h, r, c, t]);
  }
  line(x1, y1, x2, y2, c, t) {
    this.record("line", [x1, y1, x2, y2, c, t]);
  }
  polyline(pts, c, t) {
    this.record("polyline", [pts, c, t]);
  }
  fillPolygon(pts, c) {
    this.record("fillPolygon", [pts, c]);
  }
  fillCircle(cx, cy, r, c) {
    this.record("fillCircle", [cx, cy, r, c]);
  }
  fillEllipse(cx, cy, rx, ry, c) {
    this.record("fillEllipse", [cx, cy, rx, ry, c]);
  }
  drawText(x, y, str, c, o) {
    this.record("drawText", [x, y, str, c, o]);
  }
  drawImage(id, x, y, w, h, tint) {
    this.record("drawImage", [id, x, y, w, h, tint]);
  }
  textSize(str, o) {
...
class Env {
  ...
  frames(n, draw) {
    for (let i = 0; i < n; i++) this.frame(draw);
  }
  hover(x, y) {
  ...
  down(b = 0) {
    this.input.mouse.buttons[b] = true;
  }
  up(b = 0) {
    this.input.mouse.buttons[b] = false;
  }
  press(x, y, b = 0) {
    this.hover(x, y);
    this.down(b);
...
test("context menu respects window flags/state", () => {
  ...
  const labels = (p.data.items || []).map((r) => r.label).filter((l) => l);
  check(
    !labels.includes("Collapse"),
    "NoCollapse window has no Collapse, got " + labels,
  );
...
test("context menu: Collapse toggles; flag off disables the menu", () => {
  ...
  const e2 = new Env({
    flags: { animations: false, windowContextMenu: false },
  });
  const draw2 = () => {
    if (e2.gui.beginWindow("N", { pos: [200, 100], size: [300, 200] }))
      e2.gui.endWindow();
  };
  e2.frames(2, draw2);
  e2.press(300, 112, 1);
  e2.release(1);
  check(
    e2.gui.state.popupList.length === 0,
    "windowContextMenu flag off: no menu",
  );
});
...
test("covered window: right-click opens the TOP window's menu only", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [400, 200] }))
      e.gui.endWindow();
    if (
      e.gui.beginWindow("B", {
        pos: [150, 120],
  ...
  check(
    !e.gui.state.popups.get("winctx:" + B.idHash),
    "no-title B got no menu either",
  );
  e.frames(2, draw);
});
...
test("covered window: chevron click toggles only the top window", () => {
  ...
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [400, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [110, 120], size: [300, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  const A = e.gui.getWindow("A"),
...
test("item hidden under an open popup does not hover or take clicks", () => {
  ...
  const under =
    btnRect.x + btnRect.w / 2 >= p.x &&
    btnRect.x + btnRect.w / 2 < p.x + p.w &&
    btnRect.y + btnRect.h / 2 >= p.y &&
    btnRect.y + btnRect.h / 2 < p.y + p.h;
...
function vDockEnv(e) {
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [240, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [100, 340], size: [240, 200] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B", {
...
test("dragging a member header undocks it (sibling stays free)", () => {
  ...
  check(
    Math.abs(B.x - 100) < 1 && Math.abs(B.y - 273) < 1,
    "B sub-rect as expected, got " + B.x + "," + B.y,
  );
  ...
  e.frame(draw);
  e.hover(151, 285);
  e.frame(draw); // still below threshold (1px < 3px)
  check(B._dockKey != null, "no undock before the drag threshold");
  e.hover(400, 480);
  e.frame(draw); // past threshold -> undock + win-move
  e.frame(draw);
  ...
  check(
    Math.abs(A.x - 100) < 1 && Math.abs(A.y - 134) < 1,
    "A kept its sub-rect, got " + A.x + "," + A.y,
  );
  ...
  e.frames(1, draw);
  check(
    Math.abs(B.x - 350) < 1 && Math.abs(B.y - (480 - 12)) < 1,
    "B dropped at cursor, got " + B.x + "," + B.y,
  );
...
test("a pure click on a member header still toggles collapse", () => {
  ...
  check(
    e.gui.state.activeId === 0,
    "activeId reset after header click, got " + e.gui.state.activeId,
  );
});
...
test("drag-undock feeds the dock hints (drop onto another window joins it)", () => {
  ...
  const draw = () => {
    ...
    if (e.gui.beginWindow("B", { pos: [100, 340], size: [240, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("C", { pos: [600, 400], size: [240, 160] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B", {
  ...
  check(
    !!h && h.kind === "window" && h.target === e.gui.getWindow("C"),
    "grid shown over C during drag-undock",
  );
...
test("v-dock: top edge resize scales it vertically", () => {
  ...
  e.frame(draw);
  check(
    e.renderer.lastCursor === "ns-resize",
    "top edge shows ns-resize, got " + e.renderer.lastCursor,
  );
  ...
  const A = e.gui.getWindow("A");
  check(
    Math.abs(A.y - (D2.y + 34)) < 1,
    "member A followed the top edge, got y=" + A.y,
  );
  const B = e.gui.getWindow("B");
  check(
    B.h > A.h - 60,
    "members re-laid out sensibly, A.h=" + A.h + " B.h=" + B.h,
  );
});
...
test("v-dock: bottom edge + divider + move still work", () => {
  ...
  check(
    D2.ratio > 0.5,
    "divider drag changed the ratio, got " + D2.ratio.toFixed(2),
  );
  const x0 = D2.x,
    y0 = D2.y;
...
test("h-dock: vertical scaling (bottom edge) and top edge both work", () => {
  ...
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [240, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [360, 100], size: [240, 200] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B", {
  ...
  let D2 = dockRect(e);
  check(
    Math.abs(D2.h - (h0 + 80)) < 1,
    "h-dock bottom (vertical) resize, h=" + D2.h,
  );
...
test("grid pops up over a docked member; dropping splits the old dock", () => {
  ...
  const draw = () => {
    ...
    if (e.gui.beginWindow("B", { pos: [100, 340], size: [240, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("C", { pos: [600, 400], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B", {
  ...
  const h = e.gui.state._dockHint;
  check(
    !!h && h.kind === "window" && h.target === A,
    "grid over docked member A (was skipped before)",
  );
  ...
  check(
    Math.abs(box.x + box.w / 2 - cx) < 1 &&
      Math.abs(box.y + box.h / 2 - cy) < 1,
    "grid centered on A",
  );
  ...
  check(
    Math.abs(B.x - 100) < 1 && Math.abs(B.y - 273) < 1,
    "B kept its sub-rect, got " + B.x + "," + B.y,
  );
  ...
  e.down(0);
  e.frame(draw);
  e.hover(A2.x + 30, A2.y + 20);
  e.frame(draw);
  check(
    !!e.gui.state._dockHint &&
      e.gui.state._dockHint.kind === "window" &&
      e.gui.state._dockHint.target === A2,
...
test("grid pops up over a screen-edge window; dropping joins it", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("E1", { pos: [700, 100], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("F", { pos: [900, 400], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dockToEdge("E1", "left");
  e.frames(2, draw);
  const E1 = e.gui.getWindow("E1"),
    F = e.gui.getWindow("F");
  ...
  const h = e.gui.state._dockHint;
  check(
    !!h && h.kind === "window" && h.target === E1,
    "grid over edge-docked E1",
  );
  const [px, py] = e.gui._dockGridPoint(h.parts, "r");
  e.hover(px, py);
  e.frame(draw);
  ...
  check(
    E1._edge === "left" && F._edge === "left",
    "both stay in the edge stack (combined in place)",
  );
  const D = [...e.gui.state.docks.values()][0];
  check(
    !!D && D._edge === "left",
    "the combined window is itself edge-docked, got " + (D && D._edge),
  );
  check(
    D && D.dir === "v",
    "column join is forced vertical so both keep the full docked width, got " +
      (D && D.dir),
  );
  check(
    D && Math.abs(D.w - (e.gui.state.edgeDocks.left.size - 12)) <= 1,
    "combined width = the docked width (" +
      (D && D.w) +
      "), not the sum of both",
  );
});
...
test("grid is drawn (topmost) while the cursor is over the target", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("T", { pos: [500, 100], size: [240, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("S", { pos: [900, 400], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  ...
  check(
    delta >= 11,
    "join grid drawn on the hint frame (5 pads x2 + box), got " + delta,
  );
...
test("screen grid: four triangles always drawn; hovering one highlights it", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("W", { pos: [600, 400], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  ...
  e.hover(664, 360);
  e.frame(draw); // the RIGHT triangle of the center grid
  const h = e.gui.state._dockHint;
  check(
    !!h && h.kind === "screen" && h.side === "r",
    "right triangle active, got " +
      JSON.stringify(h && { k: h.kind, s: h.side }),
  );
  check(
    polys === 4,
    "square of four direction triangles drawn, got " +
      polys +
      " fillPolygon calls",
  );
  ...
  check(
    W._edge === "right",
    "drop on right triangle docked the window to the right edge, got " +
      W._edge,
  );
});
...
test("screen grid: top triangle docks top; center apex is a plain drop; triangles stack with existing edge docks", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("X", { pos: [100, 100], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("Y", { pos: [400, 100], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("Z", { pos: [700, 400], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dockToEdge("X", "left");
  e.frames(2, draw);
  const Y = e.gui.getWindow("Y"),
    Z = e.gui.getWindow("Z");
  ...
  e.frames(1, draw);
  check(
    Z._edge == null && Z._dockKey == null,
    "center apex drop kept the window free",
  );
  ...
  check(
    !!E && E.wins.length === 2 && E.wins.includes("X") && E.wins.includes("Z"),
    "Z joined the existing left edge stack with X, got " +
      JSON.stringify(E && E.wins),
  );
});
...
test("NoDock source: dragging it draws NO docking UI (no screen grid, no join grid)", () => {
  ...
  const draw = () => {
    if (
      e.gui.beginWindow("N", {
        pos: [400, 300],
        size: [200, 150],
        flags: WindowFlags.NoDock,
      })
    )
      e.gui.endWindow();
    if (e.gui.beginWindow("O", { pos: [700, 100], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  let polys = 0,
  ...
  e.hover(N.x + 100, N.y + 12);
  e.down(0);
  e.frame(draw);
  e.hover(640, 360);
  e.frame(draw); // over the screen-center grid position
  check(
    !e.gui.state._dockHint,
    "no hint at all while dragging a NoDock window",
  );
  check(
    polys === 0 && outlines === 0,
    "no grid triangles drawn (got " +
      polys +
      " fills / " +
      outlines +
      " outlines)",
  ...
  check(
    polys === 4 && outlines === 4,
    "normal drag draws the four-triangle grid, got " + polys + "/" + outlines,
  );
  e.up(0);
  e.frame(draw);
...
test("combined size without pos/size: sum of both windows in the dock direction, cross dim = the max", () => {
  const make = (dir) => {
    ...
    const draw = () => {
      if (e.gui.beginWindow("A", { pos: [100, 100], size: [240, 200] }))
        e.gui.endWindow();
      if (e.gui.beginWindow("B", { pos: [400, 150], size: [180, 120] }))
        e.gui.endWindow();
    };
    e.frames(2, draw);
  ...
  let D = make("h");
  check(
    D && D.x === 100 && D.y === 100 && D.w === 420 && D.h === 200,
    "h-dock width is the sum of both widths, got " +
      (D && JSON.stringify({ x: D.x, y: D.y, w: D.w, h: D.h })),
  );
  // vertical join: height = 200 + 120, width = the wider window (240)
  D = make("v");
  check(
    D && D.x === 100 && D.y === 100 && D.w === 240 && D.h === 320,
    "v-dock height is the sum of both heights, got " +
      (D && JSON.stringify({ x: D.x, y: D.y, w: D.w, h: D.h })),
  );
});
...
test("resizing a combined window scales the members relatively (ratios preserved)", () => {
  ...
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [240, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [340, 100], size: [360, 200] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B", {
  ...
  check(
    Math.abs(A0 - 0.4) < 0.03,
    "A starts at ~40% of the dock width, got " + A0.toFixed(3),
  );
  const w0 = D.w;
  ...
  const A2 = e.gui.getWindow("A"),
    B2 = e.gui.getWindow("B");
  check(Math.abs(D2.w - (w0 + 200)) < 1, "dock resized wider, w=" + D2.w);
  check(
    Math.abs(A2.w / D2.w - 0.4) < 0.02,
    "A still ~40% of the new width, got " + (A2.w / D2.w).toFixed(3),
  );
  check(
    Math.abs(B2.w / D2.w - 0.6) < 0.02,
    "B still ~60% of the new width, got " + (B2.w / D2.w).toFixed(3),
  );
  check(
    A2.w > A0 * w0 * 0.9 && B2.w > (1 - A0) * w0 * 0.9,
    "both members grew with the dock",
  );
});
...
test("edge stacks never cover each other (columns own the height, rows fit between)", () => {
  const make = (order) => {
    const e = new Env();
    const draw = () => {
      if (e.gui.beginWindow("L", { pos: [500, 100], size: [200, 150] }))
        e.gui.endWindow();
      if (e.gui.beginWindow("T", { pos: [500, 300], size: [240, 150] }))
        e.gui.endWindow();
      if (e.gui.beginWindow("R", { pos: [500, 500], size: [200, 150] }))
        e.gui.endWindow();
      if (e.gui.beginWindow("B", { pos: [500, 640], size: [240, 150] }))
        e.gui.endWindow();
    };
    e.frames(2, draw);
  ...
  const overlap = (e) => {
    const wins = ["L", "T", "R", "B"]
      .filter((t) => e.gui.getWindow(t) && e.gui.getWindow(t)._edge)
  ...
  check(
    T.x >= L.x + L.w,
    "top row starts after the left column, T.x=" + T.x + " vs " + (L.x + L.w),
  );
  e = make([
    ["T", "top"],
    ["L", "left"],
  ...
  check(
    !overlap(e),
    "top + left (reversed dock order) do not overlap, got " + overlap(e),
  );
  ...
  const L2 = e.gui.getWindow("L"),
    R2 = e.gui.getWindow("R"),
    T2 = e.gui.getWindow("T"),
    B2 = e.gui.getWindow("B");
  check(
    L2.y + L2.h > T2.y + T2.h - 2 && L2.y < T2.y + 2,
    "left column spans past the top row",
  );
  check(
    T2.x >= L2.x + L2.w && T2.x + T2.w <= R2.x + R2.w,
    "top row fits between the columns",
  );
  check(
    B2.x >= L2.x + L2.w && B2.x + B2.w <= R2.x + R2.w,
    "bottom row fits between the columns",
  );
});
...
test("edge stacks scale from the inner side only (resize bar method, dock direction only)", () => {
  ...
  const L = e.gui.getWindow("L");
  const innerX = () => {
    const R = e.gui.state.edgeDocks.left._rect;
    return R.x0 + R.colW;
  };
  // left column: inner (right) side is the only scalable side, ew-resize
  e.hover(innerX() - 3, 300);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "ew-resize",
    "left column inner side shows ew-resize, got " + e.renderer.lastCursor,
  );
  const bar = e.renderer.calls.filter(
    (c) =>
      c[0] === "fillRoundedRect" &&
      c[3] === 4 &&
      c[4] >= 24 &&
  ...
  check(
    bar.length >= 1,
    "resize bar drawn over the inner edge, got " + bar.length,
  );
  e.hover(2, 300);
  e.frame(draw);
  check(
    e.renderer.lastCursor !== "ew-resize",
    "screen-docked side is not scalable, got " + e.renderer.lastCursor,
  );
  const w0 = L.w,
    h0 = L.h;
  ...
  e.frames(2, draw);
  const innerY = () => {
    const R = e.gui.state.edgeDocks.top._rect;
    return R.y0 + R.colH;
  };
  e.hover(600, innerY() + 3);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "ns-resize",
    "top row inner side shows ns-resize, got " + e.renderer.lastCursor,
  );
  e.hover(600, 2);
  e.frame(draw);
  check(
    e.renderer.lastCursor !== "ns-resize",
    "top row screen side is not scalable, got " + e.renderer.lastCursor,
  );
  const T = e.gui.getWindow("T");
  const tw0 = T.w,
    th0 = T.h;
  ...
  check(
    T2.h > th0 + 20 && Math.abs(T2.w - tw0) < 2,
  ...
  e.hover(innerX() + 4, 300);
  e.frame(draw);
  check(
    e.renderer.lastCursor !== "ew-resize",
    "4px out is outside N=3, got " + e.renderer.lastCursor,
  );
  e.hover(innerX() + 2, 300);
  e.frame(draw);
  check(
    e.renderer.lastCursor === "ew-resize",
    "2px out is inside N=3, got " + e.renderer.lastCursor,
  );
});
...
test("edge-split: boundary follows the mouse 1:1 and stays while held", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [500, 100], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [500, 300], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  ...
  check(
    Math.abs(h2 - h1) < 2,
    "held stationary: A.h stable (" + h1 + " -> " + h2 + ")",
  );
  check(
    Math.abs(h1 - h0 - 60) <= 2,
    "boundary moved 1:1 with the mouse (+" + (h1 - h0) + " for +60)",
  );
  e.up(0);
  e.frame();
  ...
  e.down(0);
  e.frame();
  e.hover(100, gapY2 - 30);
  e.frame();
  const b1 = e.gui.getWindow("B").h;
  for (let i = 0; i < 10; i++) e.frame();
  check(
    Math.abs(e.gui.getWindow("B").h - b1) < 2,
    "second drag held: B.h stable",
  );
  check(
    Math.abs(b1 - hB0 - 30) <= 2,
    "second drag 1:1 (mouse -30 -> B.h +" + (b1 - hB0) + ")",
  );
  e.up(0);
  e.frame();
});
...
test("global dock UI takes input priority over windows under the cursor", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 400], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [560, 280], size: [200, 160] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  const B = e.gui.getWindow("B");
  ...
  check(
    !!h && h.kind === "screen" && h.side === "r",
    "center grid active over a window, got " +
      JSON.stringify(h && { k: h.kind, s: h.side }),
  );
  e.up(0);
  e.frame();
  e.frame();
  check(
    e.gui.getWindow("A")._edge === "right",
    "drop on the center grid over B docked A to the right edge",
  );
  ...
  e.hover(Aw.x + 50, Aw.y + 12);
  e.down(0);
  e.frame();
  check(
    !!e.gui.state.drag && e.gui.state.drag.type === "win-move",
    "A drag started",
  );
  e.hover(20, 450);
  e.frame(); // inside the left 44px band AND over B
  const h3 = e.gui.state._dockHint;
  check(
    !!h3 && h3.kind === "edge" && h3.edge === "left",
    "edge band wins over the window under the cursor, got " +
      JSON.stringify(h3 && { k: h3.kind, e: h3.edge }),
  );
  e.up(0);
  e.frame();
  e.frame();
  check(
    e.gui.getWindow("A")._edge === "left",
    "drop on the band over B docked A to the left edge",
  );
  ...
  e.hover(Aw2.x + 50, Aw2.y + 12);
  e.down(0);
  e.frame();
  e.hover(100, 440);
  e.frame(); // inside B, over the join grid's top triangle
  const h4 = e.gui.state._dockHint;
  check(
    !!h4 && h4.kind === "window" && h4.target === B && h4.side === "t",
    "join grid still works over a plain window, got " +
      JSON.stringify(h4 && { k: h4.kind, s: h4.side }),
  );
  e.up(0);
  e.frame();
  e.frame();
...
test("input only reaches the topmost element (dock chrome, edge band, modal)", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("L", { pos: [700, 100], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("P", { pos: [900, 300], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("W", { pos: [700, 400], size: [300, 200] }))
      e.gui.endWindow(); // topmost
  };
  e.frames(2, draw);
  e.gui.dock("L", "P");
  e.frames(2, draw);
  ...
  check(
    e.renderer.lastCursor !== "ew-resize",
    "no divider cursor through W, got " + e.renderer.lastCursor,
  );
  e.down(0);
  e.frame();
  check(
    !(e.gui.state.drag && e.gui.state.drag.type === "dock-split"),
    "click through W did not start dock-split, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.up(0);
  e.frame();
  ...
  check(
    e.renderer.lastCursor === "ew-resize",
    "divider cursor where uncovered, got " + e.renderer.lastCursor,
  );
  e.down(0);
  e.frame();
  check(
    !!e.gui.state.drag && e.gui.state.drag.type === "dock-split",
    "uncovered divider starts dock-split, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.up(0);
  e.frame();
  ...
  e.frame();
  check(
    e.gui.state.hoveredWindow === W,
    "W is the topmost window at the point",
  );
  check(
    e.renderer.lastCursor !== "ew-resize",
    "no edge-bar cursor through W, got " + e.renderer.lastCursor,
  );
  e.down(0);
  e.frame();
  check(
    !(e.gui.state.drag && e.gui.state.drag.type === "edge-resize"),
    "click through W did not start edge-resize, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.up(0);
  e.frame();
  ...
  check(
    e.renderer.lastCursor === "ew-resize",
    "edge-bar cursor where uncovered, got " + e.renderer.lastCursor,
  );
  e.down(0);
  e.frame();
  check(
    !!e.gui.state.drag && e.gui.state.drag.type === "edge-resize",
    "uncovered band starts edge-resize, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.up(0);
  e.frame();
  ...
  const e2 = new Env();
  const draw2 = () => {
    if (e2.gui.beginWindow("N", { pos: [100, 100], size: [200, 150] }))
      e2.gui.endWindow();
    if (
      e2.gui.beginWindow("M", {
        pos: [500, 300],
        size: [200, 150],
  ...
  const N = e2.gui.getWindow("N"),
    M = e2.gui.getWindow("M");
  e2.hover(N.x + N.w + 3, N.y + 75);
  e2.frame();
  e2.down(0);
  e2.frame();
  check(
    !(e2.gui.state.drag && e2.gui.state.drag.type === "win-resize"),
    "modal open: lower window resize band is dead, drag=" +
      (e2.gui.state.drag && e2.gui.state.drag.type),
  );
  e2.up(0);
  e2.frame();
  e2.hover(M.x + M.w + 3, M.y + 75);
  e2.frame();
  check(
    e2.renderer.lastCursor === "ew-resize",
    "the modal's own band still works, got " + e2.renderer.lastCursor,
  );
});
...
test("a dock can be globally docked (dockToEdge on a member docks the whole dock)", () => {
  ...
  const L = e.gui.getWindow("A"),
    R = e.gui.getWindow("B");
  check(
    !!r && !!D && D._edge === "left",
    "dock is edge-docked, got " + (D && D._edge),
  );
  check(L._edge === "left" && R._edge === "left", "both members flagged _edge");
  const E = e.gui.state.edgeDocks.left;
  check(
    E && E.wins.length === 1 && E.wins[0] === "A",
    "stack has ONE unit (member A), got " + JSON.stringify(E && E.wins),
  );
  check(
    D && Math.abs(D.w - (E.size - 12)) <= 1,
    "dock width = the column width, not the sum, got " + (D && D.w),
  );
  // undockEdge frees the dock as a FREE dock (the combination survives)
  e.gui.undockEdge("B");
  e.frames(2, draw);
  check(
    e.gui.isDocked("A", "B") && D && !D._edge,
    "undockEdge: the dock survives as a free dock",
  );
...
test("dragging a dock to the screen grid / edge band globally docks it", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [100, 300], size: [200, 150] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B");
  e.frames(2, draw);
  const D = e.gui.state.docks.get("A\x01B");
  e.hover(D.x + 100, D.y + 12);
  e.down(0);
  e.frame(draw);
  check(
    !!e.gui.state.drag && e.gui.state.drag.type === "dock-move",
    "dock-move started, got " + (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.hover(664, 360);
  e.frame(draw); // right triangle of the center grid
  const h = e.gui.state._dockHint;
  check(
    !!h && h.kind === "screen" && h.side === "r",
    "screen grid active while dragging a dock, got " +
      JSON.stringify(h && { k: h.kind, s: h.side }),
  );
  e.up(0);
  e.frame(draw);
  ...
  check(
    !!e.gui.state.drag && e.gui.state.drag.type === "dock-move",
    "dock movable again after undockEdge",
  );
  e.hover(20, 360);
  e.frame(draw); // inside the left 44px band
  const h2 = e.gui.state._dockHint;
  check(
    !!h2 && h2.kind === "edge" && h2.edge === "left",
    "edge band active while dragging a dock, got " +
      JSON.stringify(h2 && { k: h2.kind, e: h2.edge }),
  );
  e.up(0);
  e.frame(draw);
  e.frame(draw);
...
test("dropping a window onto a globally docked window combines in place (docked width, not the sum)", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("W", { pos: [500, 100], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("X", { pos: [700, 400], size: [300, 180] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  ...
  const h = e.gui.state._dockHint;
  check(
    !!h && h.kind === "window" && h.target === W && h.side === "t",
    "join grid over edge-docked W, got " +
      JSON.stringify(h && { k: h.kind, s: h.side }),
  );
  e.up(0);
  e.frame(draw);
  ...
  check(
    e.gui.getWindow("W")._edge === "left" &&
      e.gui.getWindow("X")._edge === "left",
    "both members stay edge-docked",
  );
  const E = e.gui.state.edgeDocks.left;
  check(
    E && E.wins.length === 1 && E.wins[0] === "W",
    "the stack holds ONE unit, got " + JSON.stringify(E && E.wins),
  );
  check(
    D && Math.abs(D.w - wW) <= 1,
    "combined width = the docked width (" +
  ...
  check(
    D && D.dir === "v",
    "column join forced vertical so both keep the full width, got " +
      (D && D.dir),
  );
  // the unit scales with the stack's inner bar, members span the full width
  const innerX =
    e.gui.state.edgeDocks.left._rect.x0 + e.gui.state.edgeDocks.left._rect.colW;
  const w0 = D.w;
  e.hover(innerX - 3, 300);
  e.down(0);
  e.frame(draw);
  e.hover(innerX + 40, 300);
  e.frame(draw);
  e.up(0);
  e.frame(draw);
  ...
  check(
    D2 && D2.w > w0 + 20,
    "dock unit scaled with the column (" + w0 + " -> " + (D2 && D2.w) + ")",
  );
  const Wa = e.gui.getWindow("W"),
    Xa = e.gui.getWindow("X");
  check(
    Math.abs(Wa.w - (D2.w - 6)) <= 1 && Math.abs(Xa.w - (D2.w - 6)) <= 1,
    "both members span the dock width (" +
      Wa.w +
      "," +
      Xa.w +
      " vs " +
...
test("dropping a window onto a docked dock splits the dock; the stack keeps the unit", () => {
  ...
  const draw = () => {
    ...
    if (e.gui.beginWindow("B", { pos: [500, 300], size: [200, 150] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("X", { pos: [700, 450], size: [300, 180] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B");
  ...
  e.down(0);
  e.frame(draw);
  e.hover(unit.x + unit.w / 2, unit.y + 30);
  e.frame(draw); // over member A
  const h = e.gui.state._dockHint;
  check(
    !!h && h.kind === "window" && h.target === unit,
    "join grid over the docked dock, got " +
      JSON.stringify(h && { k: h.kind, t: h.target && h.target.title }),
  );
  ...
  check(
    e.gui.isDocked("A", "X"),
    "X combined with the unit member A, got " +
      e.gui
        .getDocks()
        .map((d) => d.a + "+" + d.b)
        .join(","),
  ...
  const D = [...e.gui.state.docks.values()][0];
  check(
    !!D && D._edge === "left" && D.b === e.gui.getWindow("X"),
    "the new combined window is the stack unit",
  );
  check(
    e.gui.state.edgeDocks.left && e.gui.state.edgeDocks.left.wins.length === 1,
    "stack still has one unit",
  );
  const B = e.gui.getWindow("B");
  check(
    B._edge == null && B._dockKey == null,
    "the split-off member B is a free window",
  );
...
test("dock dragged over a window comes to front; header stays draggable (no softlock)", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [450, 100], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("C", { pos: [500, 300], size: [320, 220] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B");
  ...
  check(
    z.indexOf("A") > z.indexOf("C") && z.indexOf("B") > z.indexOf("C"),
  ...
  check(
    hv === A || hv === B,
    "dock title strip hovers as the dock, got " + (hv && hv.title),
  );
  e.down();
  e.frame(draw);
  check(
    e.gui.state.drag && e.gui.state.drag.type === "dock-move",
    "re-grab starts a dock-move, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.release();
  e.frames(1, draw);
  ...
  check(
    D.x === 240 && D.y === 490,
    "dock can be dragged again: " + D.x + "," + D.y,
  );
});
...
test("a window painted over a dock title strip still owns the point", () => {
  ...
  const draw = () => {
    ...
    if (e.gui.beginWindow("B", { pos: [450, 100], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("C", { pos: [150, 90], size: [200, 120] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  e.gui.dock("A", "B");
  ...
  const sx = C.x + 50,
    sy = C.y + 20;
  check(
    sy >= D.y && sy < D.y + 34 && sx >= D.x && sx < D.x + D.w,
    "point lies on the dock strip too",
  );
  e.hover(sx, sy);
  e.frame(draw);
  check(
    e.gui.state.hoveredWindow === C,
    "topmost window owns the point, got " +
      (e.gui.state.hoveredWindow && e.gui.state.hoveredWindow.title),
  );
  e.down();
  e.frame(draw);
  const d = e.gui.state.drag;
  check(
    d && d.type === "win-move" && d.win === C,
    "grab starts the window move, not a dock move, got " + (d && d.type),
  );
  e.release();
});
...
test("a dock title strip over a lower window is still grabbable", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("C", { pos: [150, 120], size: [300, 120] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [450, 100], size: [300, 200] }))
  ...
  check(
    sy >= D.y && sy < D.y + 34 && sx >= D.x && sx < D.x + D.w,
    "point on the dock strip over C",
  );
  e.hover(sx, sy);
  e.frame(draw);
  check(
    e.gui.state.hoveredWindow === A,
    "dock owns the strip point, got " +
      (e.gui.state.hoveredWindow && e.gui.state.hoveredWindow.title),
  );
  e.down();
  e.frame(draw);
  check(
    e.gui.state.drag && e.gui.state.drag.type === "dock-move",
    "dock grab works over the lower window, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.release();
});
...
test("plain window dragged over another window stays grabbable and ends on top", () => {
  ...
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [300, 150], size: [400, 300] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  const A = e.gui.getWindow("A");
  ...
  check(
    z[z.length - 1] === "A",
    "dragged window A is now drawn last (on top), z: " + z.join(","),
  );
  check(!A._dockKey, "A is still a free window (plain drop)");
  // re-grab A's title bar (overlapping B's rect)
  e.hover(A.x + 50, A.y + 12);
  e.frame(draw);
  check(
    e.gui.state.hoveredWindow === A,
    "A\u2019s title still hovers, got " +
      (e.gui.state.hoveredWindow && e.gui.state.hoveredWindow.title),
  );
  e.down();
  e.frame(draw);
  check(
    e.gui.state.drag &&
      e.gui.state.drag.type === "win-move" &&
      e.gui.state.drag.win === A,
    "A is still draggable inside B, got " +
      (e.gui.state.drag && e.gui.state.drag.type),
  );
  e.release();
});
...
test("resize band does not claim a point covered by a topmost window", () => {
  ...
  e.frame(draw);
  check(
    e.gui.state.hoveredWindow === B,
    "point belongs to B (topmost), got " +
      (e.gui.state.hoveredWindow && e.gui.state.hoveredWindow.title),
  );
  check(
    e.renderer.lastCursor !== "ew-resize" &&
      e.renderer.lastCursor !== "nwse-resize",
    "no resize cursor where B covers A\u2019s band, got " +
      e.renderer.lastCursor,
  );
  e.down();
  e.frame(draw);
  const d = e.gui.state.drag;
  check(
    !d || d.type !== "win-resize" || d.win !== A,
    "no win-resize of A from a point covered by B, got " + (d && d.type),
  );
  e.release();
  ...
  check(
    e.renderer.lastCursor === "ew-resize",
    "A\u2019s band claims once B is gone, got " + e.renderer.lastCursor,
  );
  const w0 = A.w;
  e.down();
  e.frame(draw);
  check(
    e.gui.state.drag &&
      e.gui.state.drag.type === "win-resize" &&
      e.gui.state.drag.win === A,
    "win-resize of A starts",
  );
  e.hover(522, 300);
  e.frame(draw);
...
test("join grid: the whole target window selects the side (dockJoinHitGenerous: true)", () => {
  ...
  const draw = () => {
    if (e.gui.beginWindow("A", { pos: [100, 200], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [400, 200], size: [340, 260] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  ...
  check(
    e.gui.isDocked("A", "B"),
    "drop near B\u2019s left edge combines A with B",
  );
  const D =
    e.gui.getWindow("A")._dockKey &&
    e.gui.state.docks.get(e.gui.getWindow("A")._dockKey);
  check(
    !!D && D.dir === "h" && D.a === e.gui.getWindow("A"),
    "combined side by side with A left of B (dir h), got " +
      (D && D.dir) +
      " / " +
      (D && D.a && D.a.title),
  );
...
test("screen-center grid selects a bit beyond the drawn square (default: dockScreenHitGenerous)", () => {
  ...
  check(
    A._edge === "left",
    "drop beyond the drawn grid still edge-docks, got " + A._edge,
  );
});
...
test("default join hit area is tight: only directly over a drawn triangle docks", () => {
  ...
  check(
    !e.gui.isDocked("A", "B"),
    "default join: point over the window but not over a triangle is a plain drop",
  );
  ...
  check(
    e.gui.isDocked("A", "B"),
    "default join: point directly over the left triangle combines",
  );
});
...
test("dockScreenHitGenerous: false restores the tight (drawn-square) screen grid", () => {
  ...
  check(
    A._edge == null,
    "tight screen grid: point outside the drawn square does not edge-dock, got " +
      A._edge,
  );
});
...
test("body click is handled by the topmost window without reordering the stack", () => {
  ...
  const draw = () => {
    if (
      ...
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [300, 140], size: [300, 200] }))
      e.gui.endWindow();
  };
  e.frames(2, draw);
  const A = e.gui.getWindow("A");
  ...
  check(
    A.open && !closedA,
    "click over the top window did not reach the bottom window\u2019s close button",
  );
  check(
    e.gui.state.focusedWindow === B,
    "the click focused the topmost window (B)",
  );
  check(
    e.gui.state.zOrder.map((w) => w.title).join(",") === z0.join(","),
    "click did not reorder the stack",
  );
  ...
  check(
    e.gui.state.zOrder.map((w) => w.title).join(",") === z0.join(","),
    "focusing A did not reorder the stack",
  );
  ...
  check(
    A.open && !closedA,
    "focused window below does not steal the click (close button not hit)",
  );
  check(
    e.gui.state.focusedWindow === B,
    "the overlap click was handled by the topmost window (B)",
  );
  check(
    e.gui.state.zOrder.map((w) => w.title).join(",") === z0.join(","),
    "still no reordering",
...
test("clicking a dock member body focuses it without reordering; a move raises the dock", () => {
  const e = new Env();
  const draw = () => {
    if (e.gui.beginWindow("C", { pos: [200, 250], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("A", { pos: [100, 100], size: [300, 200] }))
      e.gui.endWindow();
    if (e.gui.beginWindow("B", { pos: [450, 100], size: [300, 200] }))
  ...
  check(
    z1.join(",") === z0.join(","),
    "dock member body click did not reorder the stack (z: " +
      z1.join(",") +
      ")",
  );
  ...
  check(
    z2.join(",") === z0.join(","),
    "click over C (outside the dock) did not reorder the stack",
  );
  check(
    e.gui.state.focusedWindow === e.gui.getWindow("C"),
    "click over C (outside the dock) focused C",
  );
  ...
  check(
    z3.indexOf("A") > z3.indexOf("C") && z3.indexOf("B") > z3.indexOf("C"),
