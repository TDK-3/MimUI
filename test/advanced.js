/* header context menus, covered-element hover gating,
 * drag-member-out-to-undock, dock top-edge (vertical) scaling, dock hints
...
test('grid pops up over a docked member; dropping splits the old dock', () => {
    ...
    check(
        Math.abs(box.x + box.w / 2 - cx) < 1 && Math.abs(box.y + box.h / 2 - cy) < 1,
        'grid centered on A',
    );
...
test('grid pops up over a screen-edge window; dropping joins it', () => {
    ...
    check(
        D && D.dir === 'v',
        'column join is forced vertical so both keep the full docked width, got ' + (D && D.dir),
    );
    check(
...
test('screen grid: four triangles always drawn; hovering one highlights it', () => {
    ...
    check(
        ...
        'right triangle active, got ' + JSON.stringify(h && { k: h.kind, s: h.side }),
    );
    check(
        polys === 4,
        'square of four direction triangles drawn, got ' + polys + ' fillPolygon calls',
    );
    ...
    check(
        W._edge === 'right',
        'drop on right triangle docked the window to the right edge, got ' + W._edge,
    );
});
...
test('NoDock source: dragging it draws NO docking UI (no screen grid, no join grid)', () => {
    ...
    const draw = () => {
        if (
            e.gui.beginWindow('N', { pos: [400, 300], size: [200, 150], flags: WindowFlags.NoDock })
        )
            e.gui.endWindow();
    ...
    check(
        polys === 0 && outlines === 0,
        'no grid triangles drawn (got ' + polys + ' fills / ' + outlines + ' outlines)',
    );
    e.up(0);
    e.frame(draw);
...
test('input only reaches the topmost element (dock chrome, edge band, modal)', () => {
    ...
    const draw2 = () => {
        ...
        if (
            e2.gui.beginWindow('M', { pos: [500, 300], size: [200, 150], flags: WindowFlags.Modal })
        )
            e2.gui.endWindow();
    };
    e2.frames(2, draw2);
...
test('dropping a window onto a globally docked window combines in place (docked width, not the sum)', () => {
    ...
    check(
        ...
        'join grid over edge-docked W, got ' + JSON.stringify(h && { k: h.kind, s: h.side }),
    );
    e.up(0);
    e.frame(draw);
    e.frame(draw);
    ...
    check(
        e.gui.getWindow('W')._edge === 'left' && e.gui.getWindow('X')._edge === 'left',
    ...
    check(
        D && Math.abs(D.w - wW) <= 1,
        'combined width = the docked width (' + (D && D.w) + '), NOT the sum (' + (wW + 300) + ')',
    );
    check(
        D && D.dir === 'v',
        'column join forced vertical so both keep the full width, got ' + (D && D.dir),
    );
...
test('a dock title strip over a lower window is still grabbable', () => {
    ...
    check(
        e.gui.state.drag && e.gui.state.drag.type === 'dock-move',
        'dock grab works over the lower window, got ' + (e.gui.state.drag && e.gui.state.drag.type),
    );
    e.release();
});
...
test('plain window dragged over another window stays grabbable and ends on top', () => {
    ...
    e.frame(draw);
    check(
        e.gui.state.drag && e.gui.state.drag.type === 'win-move' && e.gui.state.drag.win === A,
...
test('resize band does not claim a point covered by a topmost window', () => {
    const e = new Env();
    const draw = () => {
        if (e.gui.beginWindow('A', { pos: [200, 200], size: [300, 200], resizable: true }))
            e.gui.endWindow();
        if (e.gui.beginWindow('B', { pos: [480, 250], size: [300, 200], resizable: true }))
            e.gui.endWindow();
    };
    e.frames(2, draw);
    const A = e.gui.getWindow('A');
    ...
    check(
        e.renderer.lastCursor !== 'ew-resize' && e.renderer.lastCursor !== 'nwse-resize',
    ...
    e.frame(draw);
    check(
        e.gui.state.drag && e.gui.state.drag.type === 'win-resize' && e.gui.state.drag.win === A,
...
test('dockScreenHitGenerous: false restores the tight (drawn-square) screen grid', () => {
    ...
    check(
        A._edge == null,
        'tight screen grid: point outside the drawn square does not edge-dock, got ' + A._edge,
    );
});
...
test('clicking a dock member body focuses it without reordering; a move raises the dock', () => {
    ...
    check(
        z1.join(',') === z0.join(','),
        'dock member body click did not reorder the stack (z: ' + z1.join(',') + ')',
    );
