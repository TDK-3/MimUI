...
require('../mim.js');
const { CanvasRenderer, CanvasInput } = require('../demo/canvas-backend.js');
...
function makeFakeCtx() {
  ...
  const ctx = {
    ...
    set font(v) { font = v; calls.push(['font', v]); },
...
function makeFakeCanvas() {
  ...
  const winEl = { addEventListener: (ev, fn) => { handlers.win[ev] = fn; } };
  return {
    clientWidth: 960, clientHeight: 540,
    width: 960, height: 540,
    getBoundingClientRect: () => ({ left: 0, top: 0, width: 960, height: 540 }),
    addEventListener: (ev, fn) => { handlers.canvas[ev] = fn; },
    __winEl: winEl,
    __handlers: handlers,
    // dispatch helpers
    mouse(ev, obj) { if (handlers.canvas[ev]) handlers.canvas[ev](obj); },
    win(ev, obj) { if (handlers.win[ev]) handlers.win[ev](obj); },
  };
}
...
console.log('• CanvasRenderer maps primitives to ctx calls');
{
  ...
  r.endFrame();
  const c = ctx.__calls;
  check(c.some((x) => x[0] === 'fillRect' && x[1] === 10 && x[2] === 20 && x[3] === 100 && x[4] === 50), 'fillRect -> ctx.fillRect');
  check(c.some((x) => x[0] === 'fillStyle' && x[1] === 'rgba(1,2,3,0.502)'), 'colors -> rgba string with alpha');
  check(c.some((x) => x[0] === 'arcTo'), 'rounded rect via arcTo path (no native roundRect in fake)');
  check(c.some((x) => x[0] === 'strokeRect'), 'strokeRect forwarded');
  check(c.some((x) => x[0] === 'lineTo'), 'line forwarded');
  check(c.filter((x) => x[0] === 'lineTo').length >= 4, 'polyline vertices forwarded');
  check(c.some((x) => x[0] === 'arc' && x[3] === 10), 'circle -> arc');
  check(c.some((x) => x[0] === 'ellipse' && x[1] === 50), 'ellipse forwarded');
  check(c.some((x) => x[0] === 'fillText' && x[1] === 'hi' && x[2] === 5 && x[3] === 5), 'text top-left anchored');
  check(c.some((x) => x[0] === 'textBaseline' && x[1] === 'top'), 'textBaseline top');
  check(c.some((x) => x[0] === 'setTransform'), 'beginFrame sets transform (DPR)');
...
console.log('• CanvasRenderer push/pop clip uses save/rect/clip/restore');
{
  ...
  check(c.some((x) => x[0] === 'clip'), 'ctx.clip called');
}
...
console.log('• CanvasInput normalizes DOM events');
{
  ...
  check(s1.mouse.buttons[1] === true && s1.mouse.buttons[0] === false, 'DOM right -> Mim right (index 1)');
  check(s1.keys.has('tab') && s1.keys.has('a') && s1.keys.has(' '), 'key tokens incl. space + letters');
...
console.log('• GUI + CanvasRenderer runs headless and responds to clicks');
{
  ...
  canvas.mouse('mousedown', evt({ button: 0, clientX: btn.x + btn.w / 2, clientY: btn.y + btn.h / 2 }));
  frame();
  canvas.win('mouseup', evt({ button: 0 }));
  frame();
  check(clicks === 1, 'button clicked via canvas input, got ' + clicks);
  check(ctx.__calls.some((x) => x[0] === 'fillText' && x[1] === 'T'), 'window title drawn through ctx');
}
...
console.log('• scroll wheel drives a child region through the GUI');
{
  ...
  const frame = () => {
    ...
    gui.beginFrame(input.snapshot()); draw(); gui.endFrame();
  };
  frame();
  ...
  check(after.some((x) => x[0] === 'fillText' && x[1] === 'row 5'), 'row 5 visible after scrolling down');
  check(after.some((x) => x[0] === 'fillText' && x[1] === 'row 8'), 'row 8 visible after scrolling down');
  check(after.some((x) => x[0] === 'fillText' && x[1] === 'row 0') === false, 'row 0 scrolled out of the child clip');
}
...
console.log('• renderer advertises features and drives the CSS cursor');
{
  ...
  frame();
  check(fakeCanvas.style.cursor === 'pointer', 'cursor set to pointer over button, got ' + fakeCanvas.style.cursor);
  fakeCanvas.win('mousemove', evt({ clientX: 5, clientY: 500 })); // empty space
  frame();
  check(fakeCanvas.style.cursor === 'default', 'cursor back to default, got ' + fakeCanvas.style.cursor);
