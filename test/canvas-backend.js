...
console.log("• CanvasRenderer maps primitives to ctx calls");
{
  ...
  check(
    c.filter((x) => x[0] === "lineTo").length >= 4,
    "polyline vertices forwarded",
  );
  check(
  ...
  check(
    c.some(
      (x) => x[0] === "fillText" && x[1] === "hi" && x[2] === 5 && x[3] === 5,
    ),
...
console.log("• CanvasInput normalizes DOM events");
{
  ...
  check(
    sMid.text === "ab",
    "typed text accumulated, got " + JSON.stringify(sMid.text),
  );
  ...
  check(
    s1.mouse.wheelY < 0,
    "wheel up is negative (Mim: positive wheelY = down), got " +
      s1.mouse.wheelY,
  );
  ...
  check(
    s2.text === "" && s2.mouse.wheelY === 0,
    "wheel/text consumed after snapshot",
  );
  // outside the canvas
...
console.log("• GUI + CanvasRenderer runs headless and responds to clicks");
{
  ...
  canvas.win(
    "mousemove",
    evt({ clientX: btn.x + btn.w / 2, clientY: btn.y + btn.h / 2 }),
  );
  canvas.mouse(
    "mousedown",
...
console.log("• scroll wheel drives a child region through the GUI");
{
  ...
  frame();
  const before = ctx.__calls.filter(
    (x) => x[0] === "fillText" && x[1] === "row 10",
  );
  check(
    before.length === 0,
    "row 10 culled before scrolling (outside child clip)",
  );
...
console.log("• renderer advertises features and drives the CSS cursor");
{
  ...
  frame();
  check(
    renderer.features && renderer.features.cursor === true,
    "features.cursor advertised",
  );
  fakeCanvas.win(
    "mousemove",
    evt({ clientX: btn.x + btn.w / 2, clientY: btn.y + btn.h / 2 }),
