...
(function (global) {
    ...
    const MouseButton = Object.freeze({ Left: 0, Right: 1, Middle: 2, Back: 3, Forward: 4 });
    ...
    class GUI {
                ...
                if (!consumed && wy !== 0) {
                    for (const p of s.popupList) {
                        if (
                            ...
                            p.maxScroll > 0 &&
                            p.w > 0 &&
                            pointInRect(s.mouse.x, s.mouse.y, { x: p.x, y: p.y, w: p.w, h: p.h })
                        ) {
            ...
            s.appMenuSections = [];
            {
                ...
                if (am) {
                    ...
                    for (const sec of am.menus) {
                        ...
                        s.appMenuSections.push({ label, rect, pid, open, items: sec.items || [] });
                    }
        ...
        beginGroup() {
            const L = this.state.layout;
            const snap = Object.assign({}, L, { origin: { ...L.origin }, scroll: { ...L.scroll } });
        ...
        _memberContextMenu(win) {
            ...
            items.push({ label: 'Undock', onActivated: () => this._undockMember(win) });
            this._openPopup(
                id,
                { x: mo.x, y: mo.y },
                { type: 'menu', items },
            ...
            for (const edge of ['top', 'bottom', 'left', 'right']) {
                ...
                for (let i = 0; i < n; i++) {
                    ...
                    if (i < n - 1) {
                        E._bounds.push(
                            horiz
                                ? { i, x: x0 + pad + off - 3, y: y0 + 1, w: gap + 6, h: colH - 2 }
                                : { i, x: x0 + 1, y: y0 + pad + off - 3, w: colW - 2, h: gap + 6 },
                        );
                        off += gap;
                    }
                }
        ...
        _dockHintUpdate(s, mo) {
            ...
            const sbHit = { x: sb.x - pad, y: sb.y - pad, w: sb.w + pad * 2, h: sb.h + pad * 2 };
            if (
                mo.x >= sbHit.x &&
            ...
            if (edge) {
                s._dockHint = { kind: 'edge', edge, band: this._edgeBandRect(edge, W, H) };
                return;
            }
        ...
        beginWindow(title, opts) {
            ...
            if (!win.noScrollbar) {
                s.scrollStack.push({
                    win,
                    rect: { x: win.x, y: win.y + win.titleH, w: win.w, h: win.h - win.titleH },
                    frame: s.frameId,
                });
            }
        ...
        _drawScrollBar(win, axis, trackX, trackY, trackLen, trackThick, content, visible) {
            ...
            const grabRect = isV
                ? { x: trackX + (trackThick - sb) / 2, y: grabStart, w: sb - 2, h: grabLen }
                : { x: grabStart, y: trackY + (trackThick - sb) / 2, w: grabLen, h: sb - 2 };
        ...
        _frameWidget(label, opts) {
            ...
            const labelRect = labelAbove
                ...
                : { x: frameX + w + sp[0], y: pos.y + (h - lineH) / 2, w: lw, h: lineH };
        ...
        listBox(label, value, items, opts) {
            ...
            if (this.beginChild('##listbox' + ids.itemId, { w: w, h: boxH, padding: 4 })) {
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    const it = this._item(p.x, p.y, rw, rowH, itemId, { focusable: false });
                    const res = this._clickable(it);
                    if (it.visible) {
        ...
        beginMenu(label) {
            ...
            if (s.currentMenu) {
                ...
                if (sub && sub.open) {
                    s.currentMenu = { popup: sub, rows: sub.data.items, _parent: s.currentMenu };
