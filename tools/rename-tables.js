...
const LOCAL_RENAMES = {
  s: {
    ...
    ok: (init) =>
      (init &&
        ...
        init.property &&
        init.property.name === "state") ||
      (init &&
        init.type === "AssignmentExpression" &&
        init.left &&
        init.left.type === "MemberExpression" &&
        init.left.object &&
        init.left.object.type === "ThisExpression" &&
        init.left.property &&
        init.left.property.name === "state"),
  },
  ...
  D: {
    ...
    ok: (init) => {
      ...
      if (init.type === "CallExpression") {
        ...
        if (isMember(c, "get") && c.object && isMember(c.object, "docks"))
          return true;
      }
      if (init.type === "ObjectExpression") {
        return init.properties.some(
          (p) => p.type === "ObjectProperty" && p.key && p.key.name === "key",
        );
      }
      return false;
    },
  ...
  it: {
    ...
    ok: (init) => {
      ...
      if (init.type === "CallExpression") {
        ...
        if (
          isMember(init.callee, "get") &&
          init.callee.object &&
          isMember(init.callee.object, "items")
        )
          return true;
      }
  ...
  tw: {
    ...
    ok: (init) =>
      ...
      isMember(init.object.callee, "_measure"),
  },
  sp: {
    to: "itemSpacing",
    ok: (init) =>
      !!(
        init &&
        init.type === "CallExpression" &&
        isMember(init.callee, "_var") &&
        init.arguments[0] &&
        init.arguments[0].value === "itemSpacing"
      ),
  },
};
