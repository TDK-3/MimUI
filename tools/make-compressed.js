...
const LOCAL_REV = {};
for (const [from, spec] of Object.entries(LOCAL_RENAMES)) LOCAL_REV[spec.to] = { to: from, ok: spec.ok };
const PARAM_REV = {};
...
(function collect() {
  (function walk(n) {
    ...
    if (n.type === 'VariableDeclarator' && n.id && n.id.type === 'Identifier' && LOCAL_REV[n.id.name] && n.init) {
...
(async () => {
  ...
  const formatted = await prettier.format(generated, { parser: 'babel', singleQuote: true, printWidth: 100 });
