...
(function (global) {
  ...
  class RendererProxy {
    ...
    pushClip(x, y, w, h) {
      ...
      this.clip =
        nr > nx && nb > ny ? { x: nx, y: ny, w: nr - nx, h: nb - ny } : null;
  ...
  class GUI {
    ...
    beginFrame(input) {
      ...
      if (s._memberDrag) {
        ...
        if (!mo.buttons[0]) {
          ...
          Math.abs(mo.x - md.x) + Math.abs(mo.y - md.y) >=
    ...
    endFrame() {
      ...
      s.modalWin = null;
      for (const w of s.zOrder)
        if (w.modal && w.drawnFrame === s.frameId) s.modalWin = w;

      // clear stale focus / active
      if (
        s.activeId !== 0 &&
        Number.isInteger(s.activeId) &&
        !s.items.has(s.activeId)
      )
        s.activeId = 0;
      ...
      st.fps =
        s.dt > 0.0005 ? lerp(st.fps || 1 / s.dt, 1 / s.dt, 0.08) : st.fps;
    ...
    popId(count = 1) {
      ...
      s.idStackSeed = s.idStackSeeds.length
        ? s.idStackSeeds[s.idStackSeeds.length - 1]
        : 0;
    ...
    _popupAtPoint(x, y) {
      ...
      for (let i = s.popupList.length - 1; i >= 0; i--) {
        ...
        if (
          p.open &&
          p.w > 0 &&
          pointInRect(x, y, { x: p.x, y: p.y, w: p.w, h: p.h })
        )
          return p;
      }
      return null;
    ...
    _item(x, y, w, h, itemId, opts) {
      ...
      const inMenuBarBand = !!(
        ...
        y + h <=
          s.layout.container.y +
            s.layout.container.titleH +
            s.layout.container.menuH
      );
      it.visible =
        w > 0 &&
        h > 0 &&
        (!clip || rectsOverlap({ x, y, w, h }, clip) || inMenuBarBand);
      it.hovered = false;
      it.active = false;
      it.clicked = false;
    ...
    _mouseIn(it) {
      ...
      return (
        mo.x >= it.x && mo.x < it.x + it.w && mo.y >= it.y && mo.y < it.y + it.h
      );
    }
    ...
    _clickable(it) {
      ...
      if (wasActive && this.isMouseReleased(0)) {
        it.clicked =
          this._mouseIn(it) || s.dragDistance < this.flags.dragThreshold;
        s.activeId = 0;
        s.activeIdWindow = null;
      }
      return {
    ...
    dockToEdge(a, edge) {
      ...
      if (w._dockKey) {
        ...
        const E =
          s.edgeDocks[edge] || (s.edgeDocks[edge] = { wins: [], fracs: {} });
        const horiz = edge === "top" || edge === "bottom";
        if (!E.size)
          E.size = horiz ? clamp(D.h, 110, 300) : clamp(D.w, 180, 420);
    ...
    _drawDockChrome(win, alpha) {
      ...
      const inRect = (x, y, w, h) =>
        mo.x >= x && mo.x < x + w && mo.y >= y && mo.y < y + h;

      if (isA) {
        const dh = D.collapsed ? tH : D.h;
        const dragging = s.drag && s.drag.dock === D;
        const focused =
          dragging || s.hoveredWindow === D.a || s.hoveredWindow === D.b;
        ...
        const title =
          D.title ||
          (D.a && D.b ? D.a.title + " +" + D.b.title : D.a ? D.a.title : "");
        this._drawText(
          D.x + 30,
          D.y + (tH - lineH) / 2 + 1,
          title,
        ...
        if (!D.collapsed) {
          ...
          if (D.dir === "h") {
            ...
            const divY =
              D.y + tH + Math.round((D.h - tH - dw) * D.ratio) + dw / 2;
            this.renderer.line(
              D.x + 3,
              divY,
              D.x + D.w - 3,
    ...
    beginWindow(title, opts) {
      opts = opts || {};
      const s = this.state;
      if (s.currentWindow && s.currentWindow.drawnFrame === s.frameId)
      ...
      if (s.drag && s.drag.win === win) {
        ...
        if (d.type === "win-move" && this.isMouseDown(0)) {
          ...
          if (d.edge & 2)
            win.w = clamp(d.w0 + (mo.x - d.mx), win.minW, win.maxW);
          if (d.edge & 8) {
            win.w = clamp(d.w0 + (d.mx - mo.x), win.minW, win.maxW);
            win.x = d.x0 + d.w0 - win.w;
          }
          if (d.edge & 1)
            win.h = clamp(d.h0 + (mo.y - d.my), win.minH, win.maxH);
          if (d.edge & 4) {
      ...
      if (
        ...
        if (
          ...
          const inClose =
            ...
            mo.x <= win.x + win.w - 6;
          const inCollapse =
            win.collapsible && mo.x >= win.x && mo.x <= win.x + 28;
      ...
      if (!win._dockKey && !win._edge) {
        ...
        const draggingResize =
          s.drag && s.drag.type === "win-resize" && s.drag.win === win;
        const edgeNow =
          win.resizable &&
          this.flags.windowResize &&
          !win.autoResize &&
      ...
      // fade-in
      if (this.flags.animations && win.createdFrame === s.frameId)
        win.alpha = 0;
      win.alpha = this.flags.animations
        ? Math.min(
            1,
            win.alpha + s.dt / Math.max(0.01, this._var("fadeDuration")),
          )
        : 1;
    ...
    _drawScrollBar(
      ...
      const dragging =
        s.drag && s.drag.type === "scroll-" + axis && s.drag.win === win;
    ...
    _drawDebugOverlay() {
      ...
      for (let i = 0; i < lines.length; i++) {
        this.renderer.drawText(
          ...
          lines[i],
          [222, 224, 230, 255],
          { fontSize: fs, fontId: this.style.font.id },
        );
      }
    }
    ...
    text(str, color) {
      ...
      if (it.visible)
        this._drawText(pos.x, pos.y, str, color || this._col("text"), fo);
    ...
    sliderFloat(label, value, vmin, vmax, fmt) {
      ...
      if (stateful)
        value = value != null ? value : st.value != null ? st.value : vmin;
      ...
      const openValuePopup = () => {
        this._openPopup(
          ...
          { x: fw.x, y: fw.y + fw.h + 2 },
          {
            ...
            label: String(label),
            value: () =>
              stateful ? (st.value != null ? st.value : vmin) : value,
      ...
      if (
        ...
        value = clamp(
          value + ((s.mouse.wheel[1] > 0 ? -1 : 1) * range) / 50,
          vmin,
          vmax,
        );
        changed = true;
    ...
    dragFloat(label, value, speed, vmin, vmax) {
      ...
      if (
        ...
        value = clamp(
          value + (s.mouse.wheel[1] > 0 ? -1 : 1) * speed,
          vmin,
          vmax,
        );
        changed = true;
        ...
        if (k("escape")) {
          ...
          if (this.flags.keyboardNavigation) {
            ...
            if (i >= 0 && list.length > 1)
              s.focusedId = list[(i + 1) % list.length];
          }
        } else if (k("backspace")) {
          pushUndo();
          if (st.sel) {
    ...
    listBox(label, value, items, opts) {
      ...
      let changed = false;
      if (
        this.beginChild("##listbox" + ids.itemId, { w: w, h: boxH, padding: 4 })
      ) {
    ...
    selectable(label, selected, opts) {
      ...
      const L = this.state.layout;
      const w =
        opts.width > 0 ? opts.width : Math.max(10, L.avail.w - L.x - L.indent);
    ...
    beginMenu(label) {
      ...
      if (it.visible)
        this._drawText(x + 10, y + 4, label, this._col("text"), fo);
      return opened;
    }
    endMenu() {
      const s = this.state;
    ...
    menuItem(label, shortcut, opts) {
      ...
      if (s.popupLayoutActive) {
        ...
        const res = this._clickable(it);
        if (res.clicked && typeof opts.onActivated === "function")
          opts.onActivated();
        if (res.clicked) {
    ...
    tableRow(values) {
      ...
      for (let i = 0; i < t.cols; i++) {
        ...
        if (this._measure(str, fo).w > t.colW[i] - 14) {
          while (
            str.length > 1 &&
            this._measure(str + "…", fo).w > t.colW[i] - 14
          )
            str = str.slice(0, -1);
          str += "…";
        }
    ...
    plotLines(label, values, opts) {
      opts = opts || {};
      if (opts.h == null)
        opts = Object.assign({}, opts, { h: this.plotHeight(opts, 60) });
      ...
      if (it.visible && values.length > 1) {
        ...
        if (it.hovered) {
          ...
          const vi = clamp(
            Math.round(frac * (values.length - 1)),
            0,
            values.length - 1,
          );
