/**
 * 3D plots: wireframe surfaces and point clouds. Drag to rotate, wheel to
 * zoom; the orientation and zoom are kept per label in the widget state.
 *
 * Options: h/share as with the 2D plots, plus spin (rad/sec auto-spin, 0
 * off) and grid (surface resolution, default 18).
 * @namespace gui.addons.t3d
 */
/**
 * A wireframe surface z = fn(x, y) with x, y in [-1, 1].
 * @function gui.addons.t3d.plot3D
 * @param {string} label
 * @param {function} fn (x, y) => z
 * @param {Object} [opts] { grid (default 18), spin, h, share }
 * @returns {Object} the plot box
 */

/**
 * A 3D point cloud. Points are [x, y, z] arrays or {x, y, z} objects;
 * non-finite/missing coords are dropped.
 * @function gui.addons.t3d.plot3DPoints
 * @param {string} label
 * @param {Array} pts [[x, y, z], ...] (or {x, y, z} objects)
 * @param {Object} [opts] { pointRadius (default 2), h, share, spin }
 * @returns {Object} the plot box
 */

/* mim_3d.js — 3D graphing addon for Mim (wireframe surfaces & point clouds).
 * ------------------------------------------------------------------------
 * STB-style, dependency-free. Load after mim.js, then use `gui.addons.t3d`:
 *
 ...
 * gui._nextPos, gui._advance, gui._drawText, gui.getRegionAvail,
