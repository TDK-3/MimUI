/**
 * Toast notifications: a top-right stack with fade in/out, type-colored
 * (info / success / error) and a time-to-live. Queue with toast(), then
 * draw() once per frame from the Foreground layer:
 * gui.layer(Mim.Layers.Foreground, function () { notifs.draw(); })
 * @namespace gui.addons.notifs
 */
/**
 * Queue a toast; returns its index. The queue is capped (oldest dropped).
 * @function gui.addons.notifs.toast
 * @param {string} msg
 * @param {Object} [opts]
 * @param {string} [opts.type] 'info' (default) | 'success' | 'error'
 * @param {number} [opts.ttl] lifetime in seconds (default 2.5)
 * @returns {number} the toast's index in the queue
 */

/**
 * Draws the queue (call once per frame, from the Foreground layer).
 * @function gui.addons.notifs.draw
 * @param {Object} [opts] { w (default 260) }
 */

/**
 * The number of toasts currently in the queue.
 * @function gui.addons.notifs.count
 * @returns {number}
 */
    ...
    return {
      ...
      draw(opts) {
        ...
        for (let i = queue.length - 1; i >= 0; i--) {
          ...
          while (
            msg.length > 6 &&
            gui._measure(msg + "\u2026", fo).w > maxW - pad * 2 - 6
          )
            msg = msg.slice(0, -1);
          if (msg !== q.msg) msg += "\u2026";
          ...
          r.fillRoundedRect(
            ...
            h,
            6,
            M.Color.withAlpha(c, Math.round(248 * a)),
          );
          r.strokeRoundedRect(
            x + 0.5,
            y + 0.5,
            ...
            h - 1,
            6,
            M.Color.withAlpha(c, Math.round(210 * a)),
            1,
          );
          const accName = TYPES[q.type] || "sliderGrab";
          const acc = accName.startsWith("#")
            ? M.Color.hex(accName)
            : gui._col(accName);
          r.fillRoundedRect(
            x + 3,
            ...
            h - 6,
            1.5,
            M.Color.withAlpha(acc, Math.round(255 * a)),
          );
          gui._drawText(
            x + pad + 3,
            y + pad / 2 - 1,
            msg,
            M.Color.withAlpha(gui._col("text"), Math.round(255 * a)),
            fo,
          );
          y += h + gap;
        }
