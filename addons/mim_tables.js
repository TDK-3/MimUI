 ...
 *           zebra striping. Returns { sorted, selected, sortCol, sortDir }.
 *
  ...
  Mim.registerAddon('tables', function (gui, M) {
    ...
    return {
      advancedTable(label, cols, rows, opts) {
        ...
        else if (opts.share > 0) h = shareHeight(opts, rowH * 2, avail.h, opts.label !== false ? lineH : 0);
        ...
        if (bodyH > 4 && gui.beginChild('##xtabbody' + label, { w: w, h: bodyH, padding: 4 })) {
          ...
          if (!sorted.length) {
            ...
            gui._drawText(p0.x + 8, p0.y + 2, 'no rows', col('textDisabled', 0.7), fo);
        ...
        return { box, sorted, selected: st.selected.slice(), sortCol: st.sortCol, sortDir: st.sortDir };
      },
    };
  });
