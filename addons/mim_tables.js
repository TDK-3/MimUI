  ...
  Mim.registerAddon('tables', function (gui, M) {
    ...
    return {
      advancedTable(label, cols, rows, opts) {
        ...
        else if (st.colWidths.length !== cols.length) // columns changed: keep known widths
        ...
        else if (opts.share > 0) h = shareHeight(opts, rowH * 2, avail.h, (opts.label !== false ? lineH : 0) + extraFilter);
        ...
        if (opts.filter) {
          ...
          st.filterText = gui.inputText('##filter' + label, st.filterText || '', { w });
        ...
        const visibleRows = needle
          ? rows.filter((row) => cols.some((c) => String(row[c.id] == null ? '' : row[c.id]).toLowerCase().includes(needle)))
          : rows;
        ...
        if (s.drag && s.drag.type === 'xtab-col' && s.drag.st === st && gui.isMouseDown(0)) {
        ...
        cols.forEach((c, ci) => {
          ...
          const it = gui._item(cell.x, cell.y, cw, headH, hashRow(label + '\x01' + c.id + '\x01h', 0), { focusable: false });
          const isSorted = st.sortCol === c.id;
          const hovEdge = mo.x >= cell.x + cw - 5 && mo.x <= cell.x + cw + 2 && mo.y >= cell.y && mo.y <= cell.y + headH;
          if (it.visible) {
            if (it.hovered && !hovEdge) r.fillRect(cell.x, cell.y, cw, headH, col('headerHovered', 0.45));
            ...
            gui._drawText(tx, cell.y + (headH - lineH) / 2 + 1, txt, col(isSorted ? 'text' : 'textDisabled'), fo);
            if (isSorted) {
              const ax = tx + tw + 8, ay = cell.y + headH / 2;
              r.polyline([ax, ay + (st.sortDir > 0 ? 2 : -2), ax + 5, ay + (st.sortDir > 0 ? -2 : 2), ax + 10, ay + (st.sortDir > 0 ? 2 : -2)],
                col('textDisabled'), 1.4);
            }
            // right-edge resize handle
            if (hovEdge) {
              r.fillRect(cell.x + cw - 1.5, cell.y + 2, 3, headH - 4, col('sliderGrab'));
          ...
          if (clicked) {
            if (hovEdge) {
              ...
              else { st.sortCol = c.id; st.sortDir = 1; }
            }
          }
          hx += cw + 4;
        });
        r.line(box.x, box.y + headH + 0.5, box.x + w, box.y + headH + 0.5, col('border'), 1);
        ...
        if (bodyH > 4 && gui.beginChild('##xtabbody' + label, { w: w, h: bodyH, padding: 4 })) {
          const bw = gui.getRegionAvail().w;
          if (!sorted.length) {
            const p0 = gui._nextPos();
            gui._drawText(p0.x + 8, p0.y + 2, needle ? 'no matching rows' : 'no rows', col('textDisabled', 0.7), fo);
          ...
          sorted.forEach((row, ri) => {
            ...
            const it = gui._item(p.x, p.y, bw, rowH, itemId, { focusable: false });
            const selected = st.selected.includes(rkey);
            if (it.visible) {
              if (selected) r.fillRect(p.x, p.y, bw, rowH, col('headerActive', 0.55));
              else if (ri % 2 === 1 && opts.zebra !== false) r.fillRect(p.x, p.y, bw, rowH, col('border', 0.07));
              if (it.hovered && !selected) r.fillRect(p.x, p.y, bw, rowH, col('headerHovered', 0.35));
              let cx = p.x + 4;
              cols.forEach((c, ci) => {
                const cw = widths[ci];
                const v = row[c.id];
                const txt = c.fmt ? String(c.fmt(v, row)) : (v == null || v === '' ? '\u2014' : String(v));
                const tw = gui._measure(txt, fo).w;
                let tx = cx + 4;
                if (c.align === 'right') tx = cx + cw - 4 - tw;
                if (c.align === 'center') tx = cx + (cw - tw) / 2;
                gui._drawText(tx, p.y + (rowH - lineH) / 2 + 1, txt, col('text'), fo);
                cx += cw + 4;
              });
            }
            gui._advance(p.x, p.y, bw, rowH);
            if (opts.selectable !== false && it.hovered && gui.isMouseClicked(0) && s.activeId === 0 && !s.drag) {
              const mod = s.keys.has('ctrl') || s.keys.has('meta') || s.keys.has('shift');
              if (mod) {
                const i = st.selected.indexOf(rkey);
                if (i >= 0) st.selected.splice(i, 1); else st.selected.push(rkey);
              } else {
        ...
        return { box, sorted, selected: st.selected.slice(), sortCol: st.sortCol, sortDir: st.sortDir, shown: sorted.length, total: rows.length };
      },
    };
  });
