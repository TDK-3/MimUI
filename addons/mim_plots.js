/**
 * 2D plots that scale with the window: draggable bezier curves, polar
 * curves, heatmaps, bar charts and multi-series line charts. All state
 * (drags, colors) is kept in the id-keyed widget state, so duplicate
 * labels in different windows are independent.
 *
 * Shared options for every plot (all optional):
 * - h: explicit height in px (legacy behavior)
 * - share: split the remaining region height evenly with share-1 sibling
 *   plots in the same window (tracked per frame)
 * - min / max: force the value range
 * - grid: draw the 5x5 grid (default true)
 * - axisLabels: min/max readout in the corners (default false)
 * - label: draw the caption above the plot (default true)
 *
 * Width always follows the window; with no h/share the plot fills the
 * remaining height. Every function returns the plot box
 * ({ x, y, w, h, inner: { x, y, w, h } }).
 * @namespace gui.addons.plots
 */
/**
 * A bezier curve through persistent control points, draggable with the
 * mouse. Points are stored normalized (0..1 inside the plot rect), so the
 * curve rescales with the window and survives resizes unchanged.
 * @function gui.addons.plots.plotBezier
 * @param {string} label
 * @param {Array|null} pts initial control points as
 *   [x0, y0, x1, y1, ...] in normalized plot coords (0..1), or null for
 *   the library-kept state (opts.defaultPts, or a built-in demo curve)
 * @param {Object} [opts] shared plot options
 */

/**
 * A polar curve r = fn(t) for t in [0, 2*pi], drawn over two guide rings.
 * @function gui.addons.plots.plotPolar
 * @param {string} label
 * @param {function} fn (t) => r (r in plot units, 1 = the outer ring)
 * @param {Object} [opts] shared plot options
 */

/**
 * A heatmap of a 2D number matrix, colored on a two-color ramp
 * (min -> c1, max -> c2); non-finite cells are drawn as gaps.
 * @function gui.addons.plots.plotHeatmap
 * @param {string} label
 * @param {Array} data number[][] (rows may be ragged)
 * @param {Object} [opts] shared plot options + c1/c2 hex colors
 */

/**
 * A bar chart. values: number[] or [label, value][] — non-finite entries
 * are dropped, zero is the baseline, negative values hang below it;
 * hovering a bar shows its value.
 * @function gui.addons.plots.plotBars
 * @param {string} label
 * @param {Array} values number[] or [label, value][]
 * @param {Object} [opts] shared plot options
 */

/**
 * A multi-series line chart with a legend; hovering shows the nearest
 * sample of every series (different lengths are fine).
 * @function gui.addons.plots.plotSeries
 * @param {string} label
 * @param {Array} series [{ name, values, color? }, ...]
 * @param {Object} [opts] shared plot options + marker (tri-state: force
 *   sample dots on/off; default: dots on short series)
 */
 ...
 *
 * Shared opts (all plot types):
 *   h / w / share / min / max  — sizing and range (see below)
 *   grid         (bool, default true)  — the 5x5 background grid
 *   axisLabels   (bool, default false) — min/max value readout in the
 *                 corners (plotBars / plotSeries)
 *   marker       (bool, for plotSeries) — force sample dots on (true) or
 *                 off (false); default: dots on short series only
 *
...
(function (root) {
  ...
  Mim.registerAddon("plots", function (gui, M) {
    ...
    /* Small min/max readout in the plot corners ({ axisLabels: true }). */
    function axisLabels(box, I, mn, mx) {
      const fo = gui._fo();
      const c = col("textDisabled", 0.9);
      gui._drawText(I.x + 2, I.y + 1, fmtNum(mx), c, fo);
      gui._drawText(I.x + 2, I.y + I.h - gui._lineH() - 1, fmtNum(mn), c, fo);
    }
    ...
    function plotFrame(label, opts) {
      ...
      // 5x5 grid (disable with { grid: false })
      if (opts.grid !== false) {
        for (let i = 1; i < 5; i++) {
          r.line(
            box.x + 3,
    ...
    return {
      ...
      plotBars(label, values, opts) {
        ...
        data.forEach((d, i) => {
          ...
          const top = Math.min(yv, y0),
            hgt = Math.max(0.5, Math.abs(yv - y0));
          const inBar =
            mo.x >= x && mo.x < x + bw && mo.y >= top && mo.y <= top + hgt;
          r.fillRect(
            x,
            top,
            bw,
            hgt,
            col(opts.color || (inBar ? "headerActive" : "sliderGrab")),
          );
          if (inBar) hov = i;
        });
        if (baseY > I.y + 0.5 && baseY < I.y + I.h - 0.5)
          r.line(I.x, baseY, I.x + I.w, baseY, col("border", 0.6), 1);
        if (hov >= 0) {
          const t =
            (typeof data[hov][0] === "string" ? data[hov][0] + ": " : "") +
            fmtNum(data[hov][1]);
          const fo = gui._fo();
          const m = gui._measure(t, fo);
          gui._drawText(box.x + box.w - m.w - 5, box.y + 2, t, col("text"), fo);
        }
        if (opts.axisLabels) axisLabels(box, I, mn, mx);
        return box;
      },
      ...
      plotSeries(label, series, opts) {
        ...
        const I = {
          ...
          y: I0.y + legendH,
          w: I0.w,
          h: Math.max(1, I0.h - legendH),
        };
        let lx = I0.x;
        clean.forEach((s0, i) => {
          const c = s0.color
            ? M.Color.hex(s0.color)
            : M.Color.hex(PALETTE[i % PALETTE.length]);
        ...
        const X = (i) =>
          I.x + (maxLen <= 1 ? I.w / 2 : (i / (maxLen - 1)) * I.w);
        ...
        clean.forEach((s0, i) => {
          ...
          let run = [];
          const flush = () => {
            if (run.length >= 4) r.polyline(run, c, 1.5);
            run = [];
          };
          // dots: on for short series by default, force with
          // { marker: true } / { marker: false }
          const showDots =
            opts.marker === true ||
            (opts.marker !== false && s0.v.length <= 24);
          s0.v.forEach((v, i2) => {
            if (!isFinite(v)) {
              flush();
              return;
            ...
            run.push(X(i2), Y(v));
            if (showDots) r.fillCircle(X(i2), Y(v), 2, c);
          });
          flush();
        });
      ...
      plotBezier(label, pts, opts) {
        ...
        if (st.ptsN) {
          ...
          st.ptsN = st.pts.map((p) => [
            clampXY(p[0] / I.w, 0, 1),
            clampXY(p[1] / I.h, 0, 1),
          ]);
          delete st.pts;
        } else if (opts.defaultPts) {
          st.ptsN = opts.defaultPts.map((p) => [
            clampXY(p[0], 0, 1),
            clampXY(p[1], 0, 1),
          ]);
        } else {
        ...
        let pts2 = pts
          ? pts.map((p) => [box.x + p[0], box.y + p[1]])
          : stored.map(toPx); // screen coords
        ...
        const samples = sampleCurve(
          pts2.map((p) => [p[0] - box.x, p[1] - box.y]),
        );
        const poly = [];
        ...
        pts2.forEach((p) => {
          ...
          r.fillCircle(
            ...
            py,
            hov ? 6 : 4.5,
            col(hov ? "headerActive" : "sliderGrab"),
          );
