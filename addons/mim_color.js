 ...
 * ------------------------------------------------------------------
 * Load after mim.js, then use `gui.addons.color`:
 *
        ...
        return {
            ...
            colorButton(label, color, opts) {
                ...
                if (it.visible) {
                    ...
                    while (shown.length > 4 && gui._measure(shown + '\u2026', fo).w > maxW)
                ...
                const clicked =
                    gui.isMouseClicked(0) && s.activeId === 0 && !s.drag && s.disabledCount === 0;
