  ...
  Mim.registerAddon('color', function (gui, M) {
    ...
    return {
      ...
      colorButton(label, color, opts) {
        ...
        if (s.drag && s.drag.type === 'color-sv' && s.drag.st === st && gui.isMouseDown(0)) {
          const rel = { x: (s.mouse.x - s.drag.px) / s.drag.pw, y: (s.mouse.y - s.drag.py) / s.drag.ph };
        ...
        if (s.drag && s.drag.type === 'color-hue' && s.drag.st === st && gui.isMouseDown(0)) {
          st.hsv[0] = (s.mouse.x - s.drag.px) / s.drag.pw < 0 ? 0 : (s.mouse.x - s.drag.px) / s.drag.pw > 1 ? 359.9 : ((s.mouse.x - s.drag.px) / s.drag.pw) * 360;
          st.hex = toHex(hsv2rgb(st.hsv[0], st.hsv[1], st.hsv[2]));
          ...
          if (hasAlpha || cur[3] < 255) {
            ...
            for (let cy2 = 0; cy2 < 2; cy2++) {
              for (let cx2 = 0; cx2 < 2; cx2++) {
                ...
                r.fillRect(swX + cx2 * 7, swY + cy2 * 7, Math.min(7, sw - cx2 * 7), Math.min(7, sw - cy2 * 7), dark ? [96, 96, 96, 255] : [200, 200, 200, 255]);
              }
            }
          }
          r.fillRoundedRect(swX, swY, sw, sw, 3, cur); // 4-component color (alpha required by the renderer)
          r.strokeRoundedRect(swX + 0.5, swY + 0.5, sw - 1, sw - 1, 3, gui._col('border'), 1);
          const tx = lx + 2;
          const maxW = swX - tx - 8;
        ...
        if (st.open) {
          ...
          if (gui.beginChild('##colorpick' + label, { w: -4, h: ch, padding: 6 })) {
            ...
            r.fillRect(pp.x, pp.y, padW, PAD_H, [hueFull[0], hueFull[1], hueFull[2], 255]);
            for (let i = 0; i < strips; i++) {
              r.fillRect(pp.x + (i * padW) / strips, pp.y, padW / strips + 1, PAD_H, [255, 255, 255, Math.round(255 * (1 - i / strips))]);
            }
            for (let i = 0; i < strips; i++) {
              r.fillRect(pp.x, pp.y + (i * PAD_H) / strips, padW, PAD_H / strips + 1, [0, 0, 0, Math.round(255 * (i / strips))]);
            }
            const curX = pp.x + st.hsv[1] * padW;
            const curY = pp.y + (1 - st.hsv[2]) * PAD_H;
            r.strokeRoundedRect(curX - 5.5, curY - 5.5, 11, 11, 5.5, [0, 0, 0, 190], 1.4);
            r.strokeRoundedRect(curX - 4, curY - 4, 8, 8, 4, [255, 255, 255, 255], 1.6);
            const inPad = s.mouse.x >= pp.x && s.mouse.x < pp.x + padW && s.mouse.y >= pp.y && s.mouse.y < pp.y + PAD_H;
            if (clicked && inPad && !s.drag) {
              st.hsv[1] = clamp01c((s.mouse.x - pp.x) / padW);
              st.hsv[2] = 1 - clamp01c((s.mouse.y - pp.y) / PAD_H);
              st.hex = toHex(hsv2rgb(st.hsv[0], st.hsv[1], st.hsv[2]));
              s.drag = { type: 'color-sv', st, px: pp.x, py: pp.y, pw: padW, ph: PAD_H };
              s.activeId = -1;
            }
            ...
            for (let i = 0; i < strips; i++) {
              ...
              r.fillRect(hp.x + (i * padW) / strips, hp.y, padW / strips + 1, HUE_H, [hueC[0], hueC[1], hueC[2], 255]);
            }
            const hueX = hp.x + (st.hsv[0] / 360) * padW;
            r.strokeRoundedRect(hueX - 2.5, hp.y - 2, 5, HUE_H + 4, 2.5, [255, 255, 255, 255], 1.6);
            const inHue = s.mouse.x >= hp.x && s.mouse.x < hp.x + padW && s.mouse.y >= hp.y - 4 && s.mouse.y < hp.y + HUE_H + 4;
            ...
            if (hasPresets) {
              ...
              for (const p of PRESETS) {
                ...
                r.fillRoundedRect(cx, cy, PRESET_CELL, PRESET_CELL, 3, M.Color.hex(p));
                r.strokeRoundedRect(cx + 0.5, cy + 0.5, PRESET_CELL - 1, PRESET_CELL - 1, 3, gui._col('border', 0.8), 1);
                const cellIt = gui._item(cx, cy, PRESET_CELL, PRESET_CELL, gui._id('##cpre' + label + p).itemId, { focusable: false });
                if (cellIt.visible && cellIt.hovered) r.strokeRoundedRect(cx + 0.5, cy + 0.5, PRESET_CELL - 1, PRESET_CELL - 1, 3, gui._col('text', 0.9), 1.4);
        ...
        // changed)
        const outA = hasAlpha
          ? (st.a == null ? cur[3] : clamp255(st.a * 255))
          : (cur[3] != null ? cur[3] : 255);
